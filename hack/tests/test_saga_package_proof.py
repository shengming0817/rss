...
SPEC = importlib.util.spec_from_file_location("proof", Path(__file__).parents[1] / "saga-package-proof.py")
proof = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(proof)


class CandidateProof(unittest.TestCase):
    def test_candidate_job_consumes_upload_artifacts(self):
        workflow = (Path(__file__).parents[2] / ".github/workflows/candidate-bundle.yml").read_text()
        command = 'python3 hack/saga-package-proof.py --artifacts "$RSS_PACKAGE_PROOF" --revision "$GITHUB_SHA"'
        self.assertIn(command, workflow)
        self.assertLess(workflow.index(command), workflow.index("Upload immutable package proof"))
