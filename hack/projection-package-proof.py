#!/usr/bin/env python3
"""Run independent projection consumers against sources or fixed candidate archives."""
import json
import os
import shutil
...
SCENARIOS = {'core': ['projection-core'], 'pg': ['projection-pg']}


def main():
    args = proof_arguments(SCENARIOS)
    selected = example_dependencies({name: SCENARIOS[name] for name in (args.scenario or SCENARIOS)})
    root, allowed = prepare_sources(args, {dep for _, deps in selected.values() for dep in deps}, 'projection-')
    for name, (features, dependencies) in selected.items():
        directory = root / name
        consumer(directory, features, dependencies, allowed)
        if name == 'core':
            shutil.copyfile(ROOT / 'crates/examples/probes/projection-core.rs', directory / 'src/main.rs')
        resolved = json.loads(cargo(['metadata', '--format-version', '1'], directory))
        forbidden = ['rss-device-command', 'rss-device-command-postgres', 'rss-reconcile', 'rss-reconcile-postgres', 'rss-saga', 'rss-saga-postgres', 'testkit']
        forbidden = set(forbidden)
        required = {'rss-projection': {'default'}}
        message = set()
        if name == 'core':
            forbidden |= {'sqlx', 'rss-projection-postgres', 'rss-transactional-messaging-postgres'}
        else:
        ...
        if name == 'core':
            ...
            print('PASS projection/core: provider-free public behavior', flush=True)
            continue
        cargo(['build', '--locked', '--bin', 'projection'], directory)
        binary = str(directory / 'target/debug/projection')
        env = dict(cargo_environment(os.environ), RSS_PROJECTION_EXAMPLE=binary,
                   RSS_TEST_RUN_ID=f'{root.name}-{name}')
        command = ['cargo', 'run', '--locked', '-p', 'testkit', '--features', 'containers', '--bin', 'rss-test-launcher', '--', '--',
                   'cargo', 'test', '--locked', '-p', 'projection-postgres-integration', '--test', 'suite',
        ...
        shutil.rmtree(directory / 'target')
        print(f'PASS projection/{name}: independent graph + real PostgreSQL behavior', flush=True)
