...
fn event_metadata_exposes_only_canonical_tenant_time_and_optional_correlation() {
    ...
    let correlation =
        rss_diag_context::CorrelationId::parse("correlation-1").expect("valid correlation");
    ...
    assert_eq!(
        metadata.correlation().map(|value| value.as_str()),
        Some("correlation-1")
    );
}

#[test]
fn event_metadata_accepts_absent_correlation() {
    let tenant = rss_request_context::TenantId::parse(TENANT).expect("canonical tenant");
