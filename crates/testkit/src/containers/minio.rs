...
const MINIO_NEIGHBOR_BUCKET: &str = "neighbor";
const MINIO_POLICY_NAME: &str = "archive-workload";
const MINIO_PORT: u16 = 9000;
const MINIO_ROOT_USER: &str = "fixture-root";
const MINIO_ROOT_PASSWORD: &str = "fixture-root-password";
const MINIO_WORKLOAD_USER: &str = "fixture-archive";
const MINIO_WORKLOAD_PASSWORD: &str = "fixture-archive-password";

// ── MinIO / S3-compatible object storage ────────────────────────────────────

pub(super) fn minio_archive_policy() -> String {
    format!(
        r#"{{"Version":"2012-10-17","Statement":[{{"Effect":"Allow","Action":["s3:GetBucketVersioning","s3:GetBucketObjectLockConfiguration","s3:GetLifecycleConfiguration"],"Resource":["arn:aws:s3:::{MINIO_ARCHIVE_BUCKET}","arn:aws:s3:::{MINIO_UNVERSIONED_BUCKET}","arn:aws:s3:::{MINIO_UNLOCKED_BUCKET}"]}},{{"Effect":"Allow","Action":["s3:GetObject","s3:GetObjectVersion","s3:GetObjectRetention","s3:PutObject","s3:PutObjectRetention"],"Resource":"arn:aws:s3:::{MINIO_ARCHIVE_BUCKET}/*"}}]}}"#
    )
}
...
impl MinioTlsFixture {
    ...
    /// Delete an expired exact version using fixture administration, never the production port.
    pub async fn delete_expired_version(&self, key: &str, version: &str) -> Result<()> {
        let target = format!("rss/{MINIO_ARCHIVE_BUCKET}/{key}");
        run_container_command(
            &self._container,
            "delete expired fixture version",
            &["mc", "--insecure", "rm", "--version-id", version, &target],
        )
        .await
    }
...
/// Starts one TLS MinIO server and provisions the exact archive-only test posture.
pub async fn minio_tls_archive(attachment: NetworkAttachment<'_>) -> Result<MinioTlsFixture> {
    ...
    .await?;
    for bucket in [MINIO_UNVERSIONED_BUCKET, MINIO_UNLOCKED_BUCKET] {
        let alias = format!("rss/{bucket}");
        run_container_command(
            &container,
            "create invalid archive posture bucket",
            &["mc", "--insecure", "mb", &alias],
        )
        .await?;
    }
    let unlocked_alias = format!("rss/{MINIO_UNLOCKED_BUCKET}");
    run_container_command(
        &container,
        "enable versioning without Object Lock",
        &["mc", "--insecure", "version", "enable", &unlocked_alias],
    )
    .await?;
    run_container_command(
        &container,
