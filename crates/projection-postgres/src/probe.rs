...
SELECT
    ...
    AND EXISTS (SELECT FROM pg_attribute WHERE attrelid='rss_projection.checkpoints'::regclass
        AND attname='definition_identity' AND atttypid='bytea'::regtype AND attnotnull AND NOT attisdropped)
    AND EXISTS (SELECT FROM pg_constraint WHERE conrelid='rss_projection.checkpoints'::regclass
