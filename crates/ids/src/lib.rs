...
mod tests {
    ...
    #[test]
    fn canonical_uuid_v4_rejects_noncanonical_or_non_v4_values() {
        for raw in [
            INVALID_UUID,
    ...
    #[test]
    fn canonical_uuid_v4_retains_typed_uuid_and_redacts_debug() {
        let value = CanonicalUuidV4::parse(VALID_UUID).expect("canonical UUIDv4");
        assert_eq!(value.to_string(), VALID_UUID);
        assert_eq!(value.as_uuid().hyphenated().to_string(), VALID_UUID);
