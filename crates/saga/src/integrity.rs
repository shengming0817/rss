...
#[non_exhaustive]
/// Rejected integrity-key or stored-fingerprint metadata.
pub enum SagaReceiptIntegrityError {
    #[error("saga receipt integrity key too short")]
    /// Integrity keys must contain at least 32 bytes.
    KeyTooShort,
    #[error("saga receipt integrity key id is invalid")]
    /// Key IDs must be nonempty, bounded canonical identifiers.
    InvalidKeyId,
    #[error("saga receipt integrity key ids must be unique")]
    /// Current and previous key IDs must be unique.
    DuplicateKeyId,
    #[error("saga receipt integrity digest length is invalid")]
    /// A stored HMAC-SHA256 digest must contain exactly 32 bytes.
    InvalidDigestLength,
}
