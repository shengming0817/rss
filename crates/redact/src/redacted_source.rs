...
//!
//! INVARIANT: REDACT-SOURCE-OPAQUE-01 { level = "Hard", exec = "native-test", source = "code" }（`Debug` / `Display` / `Error::source()` 均不暴露原始 source；
...
///
/// 内层裸 `Box<dyn Error>` 是本 newtype 的**受控持有点**（脱敏边界本身）；`Debug`/`Display`
/// 已固定 `<redacted>`、不泄漏。本类型不宣称机器强制所有下游 error wrapper 采用它。
///
