//! Minimal typed actions and ephemeral AEAD; not a production effect provider.
use ring::{
    aead,
...
impl SagaReceiptProtector for Crypto {
    async fn seal(&self, plaintext: &[u8], context: &ReceiptContext) -> Result<Ciphertext, Error> {
        ...
        Ciphertext::new("example-aes-v1".into(), encoded)
    }
    async fn open(
        &self,
        ...
        if ciphertext.key_ref() != "example-aes-v1" || ciphertext.bytes().len() < 28 {
...
pub fn protection(
    ...
    integrity: [u8; 32],
) -> Result<ReceiptProtection<Crypto>, Error> {
    let id = SagaReceiptIntegrityKeyId::parse("integrity-v1")
        .map_err(|_| Error::new(rss_saga::ErrorKind::Protection))?;
    let integrity_key = VersionedSagaReceiptIntegrityKey::from_bytes(id, integrity.to_vec())
        .map_err(|_| Error::new(rss_saga::ErrorKind::Protection))?;
    Ok(ReceiptProtection::new(
        Crypto(encryption_key),
        SagaReceiptIntegrityKeyring::new(integrity_key, vec![])
            .map_err(|_| Error::new(rss_saga::ErrorKind::Protection))?,
    ))
}
#[derive(Default)]
