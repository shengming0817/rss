//! compile-fail（#1095，INVARIANT: DIPORT-ASYNC-ARC-SEND-01 { level = "Hard", exec = "verify", source = "trybuild" }）：**所有** async DI port 的 dynosaur Send
...
//!
//! 本负例覆盖全部 12 个 async DI port（anti-vacuity 回归锁，与 `dyn_compatible_pass.rs` 端口集对称）：
...
use diport::{
    ...
    DynPdp, DynPublisher, DynRateLimiter, DynRevocationStore, DynSigner, DynSubscriber,
};
use std::sync::Arc;
