...
//! 对于 #1100，outbox 写是唯一的 durable 写，故单写即满足 OutboxFact。与业务写同事务的 **co-tx 原子性**
//! （FR-003 完整 L2，如 session 持久化与 outbox append 同一 `PgTransaction`）属 #1083——届时业务写经
//! 各域 repo + unit-of-work 接缝与 append 同事务；本 port 的单事实 emit 语义不变。
//! `consistencyLevel=OutboxFact` 的 co-tx-with-business-write 维度为已跟踪缺口（#1083）。
//!
