...
pub mod device_certificate {
    ...
    pub use crate::device_certificate::{
        ...
        AuthorizedDeviceCertificateStatusRead, CertificateAttemptAuthority,
        ...
        CertificateTransportObservation, ConditionStateBatch, CurrentCommandExpiryOutcome,
        ...
        DeviceCertificateActiveCommand, DeviceCertificateActiveCommandState,
        DeviceCertificateCommandTtl, DeviceCertificateCommandTtlError, DeviceCertificateError,
        DeviceCertificateReconciler, DeviceCertificateRepository, DeviceCertificateRepositoryError,
        DeviceCertificateRepositoryLocal, DeviceCertificateScope, DeviceCertificateStateSnapshot,
        DeviceCertificateStatusAuthorizationError, DeviceCertificateStatusEvidence,
        DeviceCertificateStatusPortEffect, DeviceCertificateStatusProjectionError,
        DeviceCertificateStatusStore, DeviceCertificateStatusStoreError,
        DeviceCertificateStatusStoreLocal, DeviceIngressApplicationLineage,
        DeviceIngressApplicationLineageAuthority, DeviceIngressApplicationReceipt,
        DeviceIngressContract, DeviceIngressDelivery, DeviceIngressDomainOutcome,
        DeviceIngressError, DeviceIngressPreparation, DeviceIngressReceiptMismatch,
        DeviceIngressRepository, DeviceIngressWrite, DeviceLatentObservationError,
...
pub use crate::domain::{
    ...
    AccountStatusSetCommand, AttributeKey, AuthOutcome, BruteForceDecision, Credential,
    ...
    DecimalValue, EqualityPredicate, GrantCredentialSecurityCommand, IdentityError,
    LoginIdentifier, LogoutAllCommand, LogoutCurrentCommand, MembershipPredicate, Operator,
    OperatorInput, OperatorInputError, OperatorRef, OrderingPredicate, POLICY_ATTR_CONTRACT_ID,
    POLICY_ATTR_PERMISSION, POLICY_ATTR_PRINCIPAL_ID, POLICY_ATTR_PRINCIPAL_KIND,
    POLICY_ATTR_RESOURCE_ID, POLICY_ATTR_TENANT_ID, POLICY_VALUE_SET_MAX_ITEMS,
    PasswordChangeCommand, PendingCredentialSecurityCommit, PipAttributeKey, PipAttributeKeyError,
...
// `DynRoleReadRepo` 承载（DI 注入走 Send wrapper）。与 diport DI port 同范式（ADR-003/ADR-004 C1）。
// 生产 postgres `PgRoleRepo` 与 in-mem 替身（`crate::internal`）已写实；本注释仅说明 Send wrapper 分层。
...
///
/// Login orchestration accepts this owner instead of two independent ports. Implementations decide
