...
async fn start_with(
    ...
    owner
        .startup()
        .unwrap()
        .stage_task_with_token(register(listener, app, "http", per));
    (addr, owner)
}

type Register =
    fn(TcpListener, Router, &'static str, Duration) -> rss_runtime::ManagedTaskRegistration;
...
async fn start(app: Router, per: Duration, total: Duration) -> (SocketAddr, ShutdownStack) {
    start_with(serve_http2_registration, app, per, total).await
}
...
async fn all_http2_streams_are_cancelled_before_later_dependency_teardown() {
    for register in constructors() {
        ...
        startup.stage_task_with_token(register(listener, app, "http", Duration::from_millis(25)));
...
#[tokio::test]
#[allow(clippy::unwrap_used)]
async fn auto_establishment_deadline_does_not_limit_admitted_h2_streams() {
    let entered = Arc::new(Notify::new());
    let release = Arc::new(Notify::new());
    let app = {
        let entered = entered.clone();
