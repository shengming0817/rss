...
SELECT (SELECT valid FROM roles)
 ...
 AND NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='rss_transactional_messaging' AND has_function_privilege(current_user,p.oid,'EXECUTE') AND p.oid<>'rss_transactional_messaging.check_execution()'::regprocedure AND NOT EXISTS(SELECT 1 FROM functions f WHERE p.oid=to_regprocedure('rss_transactional_messaging.'||f.signature)))
