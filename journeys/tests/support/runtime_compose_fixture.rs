...
const FIXTURE_GIT_SHA: &str = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
const FIXTURE_BUILD_DATE: &str = "1970-01-01T00:00:00Z";
...
impl RuntimeComposeFixture {
    async fn start() -> Result<Self> {
        ...
        let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .to_path_buf();
        let mut demo_tls = Command::new("bash");
        demo_tls.arg(repo_root.join("deploy/demo-tls/generate-demo-cas.sh"));
        command_ok(&mut demo_tls, "generate Compose demo TLS material").await?;
        ...
        fixture
            .compose_ok(
                ...
                "build runtime and migration images",
            )
            .await?;
    ...
    fn compose_command(&self) -> Command {
        ...
        command
            ...
            .env("GIT_SHA", FIXTURE_GIT_SHA)
            .env("BUILD_DATE", FIXTURE_BUILD_DATE)
    ...
    async fn assert_migration_ledger(&self) -> Result<()> {
        ...
        let rows = postgres_psql(postgres, &[
            "-U",
            "postgres",
            "-d",
            "rss",
...
async fn postgres_psql(postgres: &str, args: &[&str]) -> Result<String> {
    let mut command = Command::new("docker");
    command.args([
        "exec",
        postgres,
        "/bin/sh",
        "-ec",
        "export PGPASSWORD=\"$POSTGRES_PASSWORD\"; exec psql \"$@\"",
        "rss-psql",
    ]);
    command.args(args);
    command_stdout(&mut command, "query fixture PostgreSQL").await
}
