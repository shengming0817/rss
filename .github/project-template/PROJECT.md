...
/ship <issue>
  ...
  → ship：内置 6 维 reviewer → IN_SCOPE Cx3/Cx4 评估后自动 defer + 建 issue 跟踪（不 AskUserQuestion）→ /fix Cx1/Cx2 → 贴 pm:ship（含 defer 结果）+ 全部 deferred 留痕 → 冲突预检
...
/fix <PR#>（pr-status/needs-fix 时；可多次跑，≤3 轮自动循环）
  ...
  → triage + IN_SCOPE Cx3/Cx4 评估后自动 defer + 建 issue 跟踪（不 AskUserQuestion）+ Cx1/Cx2 修复 → 贴 pm:fix（含 defer 结果）→ 冲突预检
...
> **IN_SCOPE Cx3 自动 defer**：ship/fix 切触发 label 前，每条 IN_SCOPE Cx3（及 Cx4）不修、评估后自动 defer——按 `issues` B1 建 backlog issue 跟踪（**机器可判定 artifact**，非 Soft 口头约定），与 OOS artifact-before-trigger 同序；**全部 deferred 的 issue 已建方可切 label**，不得悬空。不 AskUserQuestion（`pri-p0` incident / 归属判不定 `validate` 失败例外停人工）。manual 与 auto context 同。
