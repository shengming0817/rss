...
> 外部 CI-capable producer 的检查结果记录；ship/fix 默认执行本地 canonical `make ci CI_BASE=<remote>/develop`，不生产或等待 pm:ci。`ci-green` 是终态（`next.agent=null`），`ci-failed` 路由至 `next.agent=human`，本评论只记录外部检查结果。
