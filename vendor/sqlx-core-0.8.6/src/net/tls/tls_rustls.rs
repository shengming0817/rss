    ...
    S: Socket,
{
    ...
    let config = if tls_config.accept_invalid_certs {
        ...
        let mut cert_store = initial_root_store(tls_config.root_cert_path.is_some());
        #[cfg(all(
            not(feature = "rss-exclusive-explicit-roots"),
            any(feature = "_tls-rustls-aws-lc-rs", feature = "_tls-rustls-ring-webpki")
        ))]
        ...
        if let Some(ca) = tls_config.root_cert_path {
            ...
            add_pem_roots(&mut cert_store, &data)?;
        }
...
#[cfg(feature = "rss-exclusive-explicit-roots")]
fn initial_root_store(has_explicit_roots: bool) -> RootCertStore {
    if has_explicit_roots {
        RootCertStore::empty()
    } else {
        certs_from_webpki()
    }
...
fn add_pem_roots(store: &mut RootCertStore, pem: &[u8]) -> Result<(), Error> {
    for result in CertificateDer::pem_slice_iter(pem) {
        let certificate = result.map_err(|_| Error::tls("invalid explicit root certificate"))?;
        store.add(certificate).map_err(Error::tls)?;
    }
    Ok(())
}
...
#[cfg(all(test, feature = "rss-exclusive-explicit-roots"))]
mod rss_exclusive_explicit_roots_tests {
    use super::*;
    use rcgen::{BasicConstraints, CertificateParams, IsCa, KeyPair};

    fn private_ca(common_name: &str) -> String {
        ...
        params
            .self_signed(&KeyPair::generate().expect("generate test key"))
            .expect("self-sign test CA")
            .pem()
    }

    #[test]
    fn explicit_roots_start_empty_while_default_trust_keeps_webpki() {
        assert_eq!(initial_root_store(true).len(), 0);
        assert_eq!(
            initial_root_store(false).len(),
            webpki_roots::TLS_SERVER_ROOTS.len()
        );
    }

    #[test]
    fn explicit_store_contains_only_the_configured_bundle() {
        let first = private_ca("rss-test-ca-one");
        let second = private_ca("rss-test-ca-two");
        let mut store = initial_root_store(true);
        add_pem_roots(&mut store, first.as_bytes()).expect("add one private root");
        assert_eq!(store.len(), 1);
        add_pem_roots(&mut store, second.as_bytes()).expect("add second private root");
