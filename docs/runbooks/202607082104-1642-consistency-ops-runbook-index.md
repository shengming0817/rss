...
| LocalTx / generic UoW settlement | `docs/runbooks/202607130312-1705-localtx-unsafe-settlement.md` | `localtx_retry_attempts_total`, `localtx_final_total`, `localtx_attempts`, `postgres_localtx_connection_quarantine_total`, `tx_settlement_final_total` | `docs/ops/localtx-alerts.rules.yaml` | same checklist | no automatic replay for unsafe settlement | `observ::LocalTxObservation` for HTTP contracts; Postgres generic runner for boundary-only internal UoW; closed boundary/retry/final-status/quarantine-stage enums; see `docs/rules/observability.md` |
