...
- **Tooling/Verification** `workspacefacts`:非发布 Cargo workspace facts owner；自身无 workspace 内部依赖，以 guppy `PackageGraph` 持有调用方提供的 all-features metadata catalog，并只经必填 `BuildSelection`（内部经 guppy `CargoSet` 解析）产生 root-specific normal build facts。公开面仅暴露 owned package/path/target、target/host workspace package-feature closure 与按需 activation path；resolver/feature/platform 输入是闭值，Guppy 类型与 lifetime 不外泄。私有字段、闭值与 owned DTO 是 Hard；catalog/selection、warning fail-closed、no-residual 及真实 shipped graph 测试是 Medium，本文只描述载体而非 enforcement。合法链路精确为 `xtask → workspacefacts → guppy`；`deny.toml` wrappers 与 `cargo xtask layer-deps` 的 WORKSPACEFACTS-CONFINEMENT-01 守集合相等及真实 source edge，业务、provider、production assembly 不得消费，xtask 也不得绕过 façade 直依赖 guppy。
...
| 分层依赖残留(无 back-path 反向边 / 兄弟域互斥 / adapter·generated scope / test-support 双向 shipped confinement / RuntimeExec、authmint、workspacefacts/guppy 精确闭包 / wrappers⟷源一致) | `cargo xtask layer-deps`(source-centric：读各成员 Cargo.toml shipped 依赖表按 §分层矩阵及各 exact confinement 校验；接入 `verify`；符号/规则/盲区见 `xtask/src/layerdeps.rs` rustdoc) | Medium(CI 门) |
...
| active LocalOnly ↔ source receipt exact-set、唯一性与逐 site marker/ID/mounted ROUTE proof/三维 observers/同一 routes finalize+tuple factory/generated GET operation 闭合 AST 证书 | `cargo xtask consistency local-only-effects`（LOCAL-ONLY-RECEIPT-COVERAGE-01；module/cfg-aware synthetic red + real workspace non-empty anti-vacuity；missing fail-closed）；posture artifact 只输出 breaking JSON schema v4 | Medium(CI 门) |
| PostgreSQL tenant transaction / exact lane 与 reader schema ACL | Hard sealed `TenantDb<Lane>` + private-mint `TenantTx<Lane>` 绑定 tenant identity 与 transaction lifecycle；原始事务只在 `cotx` 内核可见，closure 仅获得不可互换的 `IdentityTx` / `SecretTx` / `EventingTx<Concern>` / `ReconcileTx` 等 operation set。类型层拒绝跨 concern、raw executor/connection/settlement、parallel tenant bind 及 lane 交叉（POSTGRES-TX-TYPE-01 / PG-TX-CONCERN-CAPABILITY-01）。`cargo xtask pg-tenant-tx-guard` 只保留类型无法证明的 raw-pool/tenant-table、SQL owner、GUC/RLS 与 settlement/quarantine 跨文件事实；`cargo xtask schema-rls` 要求 reader SELECT 同迁移、拒绝 DML/default privileges（TENANCY-PG-READER-ACL-01） | Hard + Medium(CI 门) |
