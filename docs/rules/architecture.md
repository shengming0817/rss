...
| 残留真要 AST 级的少数 funnel(某 callsite) | `dylint`(自写 clippy lint)；首条 `rss_domain_no_serialize`(domain 实体禁 derive serde `Serialize`/`Deserialize`，INVARIANT SERDE-DOMAIN-FREEZE-01)——符号/红例/盲区见 `lints/rss_domain_no_serialize/` rustdoc，运行 `cargo dylint --all` |
| 治理脚本入口 | `cargo` + `xtask/` |
