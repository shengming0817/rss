...
| command + workflow sealed seam 编译边 | `xtask/src/layers.rs` `eventexec_generated_seam_allows` 只放行 `eventexec → generated`；`authn/bootstrap/其它 Service → generated` 与反向边均保持 `GeneratedScope` 红。`deny.toml` generated wrapper 同步只增加 eventexec；正例、其它 Service 反例与真实 workspace green 三重 anti-vacuity。Command 类型/可见性 Hard seal 见 ADR-016；workflow activation 由字段私有的 runtime plan/view seal 收口。 |
