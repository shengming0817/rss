...
| `crates/devicesecuritycontracts` / `devicesecuritycontracts`（Draft candidate，已物化） | `rss-device-security-contracts` | `github:shengming0817:rss-maintainers` |
...
│   ├── pkiauthmint/      # Vault-only external-PKI closure/artifact mint capability（EXTERNAL-PKI-PROVIDER-MINT-01）
...
│   ├── devicesecuritycontracts/ # 六个 device-security Draft 契约的 generated public candidate
