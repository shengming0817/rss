...
│   ├── workspacefacts/   # Tooling/Verification：guppy PackageGraph 薄适配；owned Cargo workspace facts
...
- **Tooling/Verification** `workspacefacts`:非发布 Cargo workspace facts owner；自身无 workspace 内部依赖，只以 guppy `PackageGraph` 查询调用方提供的完整 metadata JSON，并暴露 owned package/path/target facts。合法链路精确为 `xtask → workspacefacts → guppy`；`deny.toml` wrappers 与 `cargo xtask layer-deps` 的 WORKSPACEFACTS-CONFINEMENT-01 守集合相等及真实 source edge，业务、provider、production assembly 不得消费，xtask 也不得绕过 façade 直依赖 guppy。
