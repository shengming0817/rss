...
| 残留真要 AST 级的少数 funnel(某 callsite) | `dylint`(自写 clippy lint)。已落地：① `rss_domain_no_serialize`(domain 实体禁 derive serde `Serialize`/`Deserialize`，INVARIANT SERDE-DOMAIN-FREEZE-01；完整域 crate 覆盖待 #1054)、② `rss_spawn_missing_scope`(`tokio::spawn`/`spawn_blocking` 子任务读 `runctx::try_*` 未在外层 `runctx::scope` 重绑，INVARIANT SPAWN-CTX-REBIND-01；#1031)、③ `rss_crosstenant_callsite`(vocab 跨租户 All-scope mint 三步 `CrossTenantCapability::issue_for_verified_super_admin`/`CrossTenantVisibility::authorize`/`RowVisibility::new_cross_tenant` 仅 `authn` crate 可调用——跨租户 capability 签发 callsite-allowlist 下游约束,上游私有 `_seal` 字段在 vocab 为 Hard,INVARIANT TENANCY-CROSSTENANT-CAP-01；#1074)、④ `rss_dlq_operator_callsite`(`eventexec::OperatorDlqCapability::issue_for_authorized_operator` 仅 admin/PDP 边界可调用——DLQ mutation capability 签发 callsite-allowlist,上游私有 `_seal` 字段为 Hard,INVARIANT EVENTBUS-DLQ-OPERATOR-CAP-01)、⑤ `rss_redact_debug_required`(issue #1359 高风险敏感 DTO 禁裸 `derive(Debug)`，要求 `#[derive(secure::Redact)]` 字段级脱敏，INVARIANT REDACT-DEBUG-REQUIRED-01)、⑥ `rss_diport_dto_debug_redacted`(issue #1155 `diport` DTO 字节 payload 字段禁裸 `Vec<u8>`/`[u8;N]`/`Box<[u8]>`，要求 `RedactedBytes` 脱敏 newtype——下游 Medium，配对上游 Hard 类型 `RedactedBytes`(`Debug`/`Display` 恒脱敏)，与 error 侧 `rss_diport_error_debug_redacted`+`RedactedSource` 对称，INVARIANT DIPORT-DTO-RAWBYTES-BAN-01 / 上游 DIPORT-DTO-BYTES-REDACT-01，ADR-013)——符号/红例/盲区见各 `lints/<lint>/` rustdoc；`cargo dylint --all` 已是 `cargo xtask verify` 一步并经 `DYLINT_RUSTFLAGS=-D warnings` fail-closed（#1023 完成） |
| 治理脚本入口 | `cargo` + `xtask/` |
