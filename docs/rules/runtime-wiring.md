...
- `exactExceptions` is intentionally closed. The current set is exactly `Arc<dyn distributed::DomainTransport>`, `Arc<oidc::OidcProvider>`, and `Arc<vault::VaultSigner>`; it does not allow an entire root or any other wrapper/bound.
