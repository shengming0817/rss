...
- `exactExceptions` is intentionally closed, and `xtask/runtime-deps-guard.toml` is the only source of its exact current members. Do not copy that member list into prose: every exception must name one concrete carrier shape, never a generic/mixed provider, an entire root, or an open wrapper/bound. The password digest set remains a concrete immutable `secure` value; loading stays in the crypto adapter and does not widen its exception to a provider trait or adapter-owned alias.
