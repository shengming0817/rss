# Runtime Assembly Plan Rules

This document governs the runtime assembly optimization series rooted at `docs/spec/001-runtime-assembly-plan/`. It supplements the architecture single source in `docs/rules/architecture.md`; it does not replace crate graph, contract, tenancy, observability, or eventbus rules.

## Scope Boundaries

### Phase 0

- Only create planning artifacts and deterministic governance carriers.
- No runtime behavior change.
- No assembly schema expansion before its canonical validation carrier exists.
- No generated runtime module output before the generator PR exists.

### Phase 1

- Decompose `runtime::run()` by phase without behavior change.
- Move-only PRs may move functions and update imports, module declarations, and test paths.
- Move-only PRs must not change auth policy, listener shape, provider selection, shutdown order, readiness semantics, event topology, or error handling.

### Phase 2

- Guard `SharedRuntimeDeps` against becoming a domain service locator.
- Infra-only wiring is enforced by `WIRING-DEPS-INFRA-ONLY-01` via `cargo xtask runtime-deps guard`; its allowlist and extension flow are governed by `docs/rules/runtime-wiring.md`.
- Changes to the guard must keep a Medium or Hard carrier plus red/green tests.

### Phase 3

- Assembly manifest schema v2 must declare static assembly intent through required `schemaVersion = 2`, `name`, `profile`, `domains`, `topology`, `frameworkContracts`, `workflowActivations`, `listeners`, and `diportProviders` fields. Every listener must explicitly carry its `domains` (including `[]`), every provider must explicitly carry its closed `outputs` channel set (including `[]`), and every assembly must explicitly carry its assembly-local workflow activation set (including `[]`).
- Provider `id` and `consumer` are closed `ProviderRole` and `ProviderConsumer` values. One private role registry binds every role to its lifecycle, port, constructor, provider crate, required features, consumer, durability, optional scope and failure posture, complete output set, and optional factory symbol. Active roles require exactly one factory; draft roles cannot carry one. There is no free-form role, consumer, factory path, alias, or unknown extension point.
- Contract lifecycle describes a repository definition; `WorkflowActivation` describes one assembly's deployment intent. Projection activation is closed to `disabled | capture-only | shadow | active`, Saga activation to `disabled | active`. Each activation binds the exact definition ID, version, and schema digest, and repository verification rejects unknown definitions, mode mismatches, digest drift, and invalid lifecycle × activation combinations. Topology and provider declarations do not supply an activation default.
- Assembly manifest v2, AssemblyLock v2, and RuntimePlan v3 are one protocol chain. RuntimePlan v3 adds the closed provider activation value derived from the mandatory `ProviderRole` registry; v2 readers and compatibility dispatch do not exist. The lock binds the canonical manifest to the verified repository definition; `RuntimePlan::compile_v3` requires a private-field `RepositoryVerifiedAssemblyLock`, and workflows/providers (including activation) must exactly match their canonical sources. Cargo build success, `include_bytes!`, and raw-byte equality do not mint that capability. Executable assemblies embed a deterministic `RepositoryAssemblySnapshotV2` containing the exact governed manifest, lock, generated, contract, schema, and component sources; its strict runtime reader replays the full repository join before exposing the canonical manifest and verified lock. Like path-based discovery, this verification proves completeness relative to the presented source universe; build capture binds that universe to the specific checkout and rechecks every source class before emission. Executable JSON readers bind the wire value to those values and recompile through the same funnel.
- Startup consumes `RuntimePlan` into one private `PlacedRuntimePlan` before any domain-local configuration or provider construction. Its local-domain, listener, provider, event, process security-root and Remote transport projections are minted together and then consumed by phase typestate. Generated module glue owns the manifest-ordered local-input enum, materializer and dispatch; no handwritten domain directory may precede it. Local domains alone materialize module inputs, routes, subscriptions and domain-owned providers; Remote domains alone materialize typed HTTPS/SPIFFE targets. The process security root remains active independently of Identity placement so local/framework protected routes stay fail-closed; Identity-owned modules, providers and workers do not inherit that process activation. Provider transaction receipts and inventory must match the projected provider set exactly, with LIFO-once rollback preserving the primary error.
- Production workflow activation is compiled exactly once by the private-field `eventexec::WorkflowRuntimePlan`, joining the sealed RuntimePlan to generated definitions and typed capabilities. PostgreSQL capture, Projection operator/DLQ, Saga runtime, and runtime inventory consume only borrowed views minted by that plan. Omitted or disabled workflows have no registry, provider, worker, route, probe, capture, or inventory side effect; direct production consumption of raw generated workflow catalogs is forbidden outside the compiler.
- `scope` and `failurePosture` are closed manifest validation facts, not runtime selection switches. The active `service-token-replay-store` role requires `scope = "cluster-global"` and `failurePosture = "fail-closed"` in addition to a persistent PostgreSQL provider. Missing facts, `process-local`, or `fail-open` fail `PdpReplayStoreCapability` validation before assembly generation; the assembly graph exposes the selected posture for review.
- `domains`, `topology`, and `listeners` remain declaration inputs and are projected through generated typed catalogs; they do not replace contracts, Cargo dependencies, env/secrets, listener bind config, or Rust constructor wiring.
- Manifest intent validation is carried by `ASSEMBLY-MANIFEST-INTENT-01` in `xtask/src/assembly.rs` and runs through `cargo xtask assembly validate`, `cargo xtask verify`, and CI.
- Domain required capability validation is carried by `ASSEMBLY-REQUIRED-CAPABILITY-01` in `xtask/src/assembly.rs`: declared domains/topology must have the minimum provider/dependency facts needed by generated live composition.
- Domain closure validation is evaluated per assembly target package from command-scoped `WorkspaceFacts` / CargoSet selections. Resolver-v2 Target-side All Features proves active direct edges and the inactive closure; Target-side Default proves the deployable closure and rejects `test-support`. Host edges never satisfy a domain dependency. Unknown, ambiguous, unresolved, or incomplete graph facts fail closed; workspace-wide feature unification remains a separate CI compile surface.
- This phase must not make runtime read `assembly.toml` to decide topology, route mounting, auth scheme, provider construction, or live readiness.

### Phase 4

- Domain construction converges on private-field `bootstrap::DomainBinding` values created by `DomainBinding::new`; do not introduce a second runtime output type, DI container, generic service bag, `Any`, or compatibility alias.
- `compose_bindings` is the only public output transition: it borrows domains in declared manifest order, drains and extends outputs only after compose succeeds, and leaves bindings/outputs unchanged on failure.
- Integration-only access-listener fixtures consume a private-field, move-only security-root-wired registry minted only by the production authorizer transition. Raw `WriteAdmittedRegistry`, manual authorizer injection, mutable raw-registry escape, and reusable finalization are not supported.
- `DomainModuleResult` remains the sole probes/resources/workers output. Merge/extend preserves manifest order and each domain's internal order, including duplicates; generators must not lexically sort domains.
- Domain services and routes remain typed and are captured by the domain/route funnel; they must not enter `SharedRuntimeDeps` or `DomainModuleResult`.
- `cargo xtask assembly generate-modules` first compiles `assembly.toml` into `CanonicalAssemblyManifestV2`, then derives each assembly's committed `src/generated/modules_gen.rs` from that sole semantic view. Domain/listener/framework-contract order remains semantic; `diportProviders` sort by `(port, provider, providerCrate, consumer)`, and provider `requiredFeatures`/`outputs` are duplicate-free sorted sets. The generated provenance header carries `Source-Manifest-Digest`, never a raw-TOML hash, so key/table/set order cannot create false drift. `ASSEMBLY-MODULES-CODEGEN-01` is the Hard codegen/golden carrier; `--check` runs in verify/CI and fails on missing, changed, hand-edited, or owned orphan outputs.
- The generated file contains the ordered async `domains::<name>::module(deps)` calls, typed domain-listener/provider-output evidence, plus a `#[cfg(test)]` hermetic factory emitted by the same manifest plan. It does not derive environment access, runtime instances, features, or compose/merge behavior; the test factory also returns ordinary `DomainBinding` values.
- `cargo xtask assembly generate-providers` independently derives committed `src/generated/providers_gen.rs` files, sorted by role ID and containing active entries only. Every catalog entry explicitly supplies typed role, port, constructor, factory symbol, provider crate/features, consumer, durability, optional scope/failure posture, and the complete output set (including `[]`) to `ProviderCatalogEntry::checked`. The catalog portion remains data-only; the same typed provider IR may additionally generate closed lifecycle carriers and reference-assembly transaction types with an exact AST grammar. Generated code cannot access environment/config/secrets, construct provider instances, use `Any`/`TypeId`/maps/dynamic callbacks, or introduce a service locator.
- The provider catalog is compiled privately into each assembly crate and is not an external SDK/API. The provider gate parses each crate root and requires one unconditional private `providers_gen` module plus one unconditional non-empty const assertion; deleting or `cfg`-disabling the compile link fails before generation. `ProviderCapabilityEvidence` cannot be arbitrarily constructed; the role declaration cardinality, registry tuple, and constructor feature/crate/port/durability/scope/failure-posture facts are const-checked, and the const catalog entry forms the Hard carrier. The generator validates an exact AST grammar rather than relying on a token denylist, and its filesystem planner scans the complete ownership-marker universe without following symlinks. Manifest validation, committed goldens, independent `--check`, and rustc typecheck are the Medium/codegen backstop. The aggregate order is fixed as `assembly validate → artifacts check → modules check → providers check → lock check → runtime-plan check`, and both generated families enter the AssemblyLock byte universe; the artifact matrix does not. On-demand JSON/Mermaid under `target/xtask/` are presentation only and never participate in verify/CI verdicts; their builder may reuse canonical validation but is not an independent oracle.
- `modules_gen.rs` remains the current live output-composition carrier. It is neither copied into the role registry nor accepted as a provider-catalog fallback. #1792 owns live catalog dispatch, handwritten bypass deletion, and the output bijection; #1791 does not instantiate providers.
- Defining the binding/output shape does not itself change live `runtime::run()` behavior. Moving wire functions, generating the module list, and switching the live path remain separate dependent changes with their canonical typed, generated, and behavioral verification.
- The generated artifact is the live domain-order carrier. `RUNTIME-PLAN-LIVE-CLOSURE-01` is behavior-owned by the typed exact-relation tests in `plan/domain_exec.rs`; `RUNTIME-PLAN-BINDING-BYPASS-01` rejects only cross-file handwritten wire/compose/catalog and alternate activation escapes. Phase and listener handoffs remain owned by `RUNTIME-PHASE-TRANSITION-01` and `RUNTIME-LISTENER-PLAN-EXECUTION-01`; typed route/subscriber declaration funnels preserve the single `RouteAuthorizer` / `SettingsService` instances without a service bag.
- Reusable `composition/*` crates may own a domain's typed provider-to-`DomainBinding` construction when multiple assemblies consume it. They remain Root-layer code, use mandatory typed inputs, and must not introduce a DI container, generic bag, manifest reader, or launch entrypoint.
- `settingsonly` and `identityaudit` remain executable Reference Extension assemblies on the shared `runtimeexec` launch/signal/drain kernel during the product-profile transition. Their closed configuration schemas, image stages, Health listener/inventory ownership, and existing provider, authentication, readiness, and behavior guards remain valid regression carriers, but they are frozen from adding product capabilities, provider combinations or independent T3 scope. Their eventual move out of the core release surface follows ADR-024 and requires a separate migration decision. `topology = "demo"` still does not claim production durability or the full-runtime production posture.
- #1904 adds `deviceidentity` only as a library composition proof. Its canonical manifest is exactly `profile = "demo"`, `domains = ["identity"]`, `topology = "demo"`, with empty listener/framework-contract/workflow-activation sets and the active persistent pilot roles accepted by the closed registry. The manifest and registry remain the sole provider inventory; this rule does not duplicate their role list. The draft DeviceLatent proposal set remains draft and unmounted.

## Assembly Definition of Done

- Every direct `assemblies/*` target is classified exactly once in schema-v1 `assemblies/artifacts.toml`; an unknown, omitted, duplicate, or ghost target fails closed. Artifact-matrix versioning is independent of assembly manifest/lock/RuntimePlan v3.
- A `supported` assembly closes five artifact identities: an exact Cargo binary target, a unique nonroot distroless Docker stage with matching entrypoint, a validated configuration carrier, exact `runtimeexec` Health listener/inventory ownership, and a non-vacuous acceptance journey. `runtime` uses the typed environment catalog guarded by the runtime-env gate; `settingsonly` and `identityaudit` use recursively closed Draft-07 JSON schemas. Artifact completeness does not authorize T3: only a GA-hardening-authorized candidate or activated official product profile may designate its candidate/canonical artifact and closed T3 owner; every other journey stays at its lowest sufficient T1/T2 or frozen transition layer.
- `compile-only` is an explicit lifecycle with a non-empty reason and cannot carry deployable artifact fields. There is no default lifecycle, compatibility alias, fallback reader, report-only mode, or alternate CLI.
- The #1904 `deviceidentity` row is `compile-only`: it has no binary, image, configuration carrier, Health inventory, journey, listener, or deployment projection. Its static closure does not satisfy #1906 convergence evidence, #1908 broker/backpressure fault evidence, ADR-028's future typed candidate closure/component lifecycle, or the later production process join and activation handoff.
- The Assembly Governance IR owns the closed production identity ratchet and requires its current exact set to remain `supported` with a non-vacuous journey until an ADR-024 migration or canonical-carrier replacement is completed. New non-production targets may be explicit `compile-only` rows, but changing both profile and artifact lifecycle cannot downgrade an existing production carrier. Keeping a row `supported` during this transition preserves artifact regression evidence; it does not create or extend an independent T3 product owner.
- `ASSEMBLY-ARTIFACT-MATRIX-01` in `xtask/src/assembly_artifacts.rs` is the Medium enforcement carrier. It validates application-owned binary, image, configuration, health/inventory, and journey facts and returns only a verified supported-row count; external delivery/release tooling owns every deployment projection. `cargo xtask assembly artifacts check` is the focused gate, and typed verify/CI execute it after manifest validation.
- Artifact classification is an application validation boundary, not a deployment plan. It is not an input to canonical `AssemblyManifest`, generated modules/providers, AssemblyLock, or RuntimePlan and therefore cannot rotate those identities. Its internal Health inventory is not #1806's authorized public RuntimeInventory, and a supported row is not #1801 production-posture evidence.

## PR Size Planning Budget

- This section is a planning and review budget, not a current `INVARIANT:` or machine-enforced merge gate.
- A later CI governance PR must add a Medium carrier before this budget becomes blocking enforcement.
- Normal PR target: no more than 2000 net changed lines.
- PRs above that target should name one exception class in the PR body:
  - move-only runtime split,
  - generated module output,
  - PG provider bundle standardization,
  - CI governance wiring.
- Exception notes should state the reason, affected file family, and validation command.
- A PR that mixes an exception with unrelated behavior changes is outside this plan and should be split.

## Non-Negotiable Architecture Rules

- Domains remain isolated bounded-context crates.
- Cross-domain communication remains contract-driven through `contracts/**` and `generated`.
- `assembly.toml` is a deployment/provider fact source; it does not become a wire contract source.
- Runtime assembly may depend on adapters and domains as a composition root, but domain crates must not depend on adapters or sibling domains.
- Provider declarations must remain machine-readable and validated against assembly dependencies, required features, lifecycle, durability, scope, failure posture, and consumer.
- Provider catalog changes are code-only. They require no database, secret, configuration, or external wire-contract migration. Rollback reverts the binary/catalog/lock/fingerprint diff as one unit; it must not add an alias, old reader, dual write, free-form factory path, or runtime fallback.

## AI-HARD Policy

- New enforcement claims must have a Hard or Medium carrier.
- Do not add Soft-only governance rules.
- A documentation-only statement may describe a desired future guard, but it must not use `INVARIANT:` until the carrier exists.
- Every new Medium gate must include an anti-vacuity red case or fixture.
- Every generated artifact must have a drift check before later PRs depend on it.

The provider catalog design follows
[Typify's programmatic code generation](https://github.com/oxidecomputer/typify/blob/aec3da53c4319164542b393a86d552424be24384/typify/src/lib.rs)
and [Omicron's explicit package manifest](https://github.com/oxidecomputer/omicron/blob/9932e3633a3417d130af44dfce12672eb8e0ec00/package-manifest.toml)
as comparison points. RSS adopts closed programmatic output and explicit manifest identity, not
their public API or package model.

## Security Production Closeout Gate

INVARIANT: SECURITY-PRODUCTION-CLOSEOUT-01 { level = "Medium", exec = "check", source = "code" }.

- For `profile = "production"`, `assembly validate` must require active persistent backend providers for `oidc::OidcProvider`, `vault::VaultSigner`, and `vault::VaultKeyProvider`.
- For `profile = "production"`, every declared provider must be active and persistent. Listener rate limiting is the cluster-global Redis provider; there is no replica-local ephemeral exception. Full runtime additionally requires the persistent PostgreSQL revocation store and Vault secret resolver. Required Cargo features are derived from the closed provider constructor registry rather than duplicated in production policy.
- For `profile = "production"`, typed generated catalogs, move-only provider batches, closed lifecycle outputs, receipt generation and unconsumed-output rejection are the Hard owners. Assembly validation retains only the production-reachable join from generated provider selection through construction/finish/transfer into inventory publication, plus profile-specific JWKS and managed-resource semantics that have no lower typed owner. It does not certify helper names, exact bodies, local ordering, target counts, or journey source shape.
- The active `ListenerPdp` role generates one private `ListenerPdpJwksLifecycle` shape in every consuming assembly. Its non-empty `head + tail` representation pairs each readiness probe with exactly one managed resource, materializes zero workers, and exposes only `single`, `merge`, and consuming `into_output`; rustc plus `ASSEMBLY-PROVIDERS-CODEGEN-01` own these Hard facts. Runtime and the reference assemblies retain their existing generic transaction owners, while `SECURITY-PRODUCTION-CLOSEOUT-01` remains the Medium owner only for production reachability and JWKS-specific semantic evidence that cannot be expressed by the carrier.
- For `profile = "production"`, `assembly validate` must find `run()`-reachable Rust AST evidence for SPIFFE/mTLS Internal/domain transport wiring and must reject legacy Internal service-token migration constants.
- For production `assemblies/runtime`, `assembly validate` must keep egress TLS downgrade knobs (`RSS_AMQP_ALLOW_PLAINTEXT`, `RSS_REDIS_ALLOW_PLAINTEXT`, `RSS_S3_ALLOW_PLAINTEXT`, `RSS_PG_SSL_MODE`) inside `FORBIDDEN_SERVING_KEYS` and out of `FIXED_SERVING_KEYS`. Ingress `RSS_LISTENER_ALLOW_PLAINTEXT` remains allowed. When runtime Redis/AMQP/S3 wiring sources exist, production AST must reference the private-CA funnels (`RedisPrivateCa` / `AmqpPrivateCa` / `PrivateCaS3ClientFactory` / `connect_with_private_ca`).
- Comment, string, dead-helper, and `#[cfg(test)]` bait are not production evidence. Medium residuals are organized by canonical failure mode—bypassed live construction, detached listener address, missing JWKS/SPIFFE reachability, and egress TLS/private-CA drift—and each keeps a synthetic RED plus a real-workspace anti-vacuity check.
- `profile = "demo"` assemblies are not blocked by this production-only closeout gate.

## Runtime Assembly Native and Residual Policy

- `assemblies/runtime/Cargo.toml` is the sole runtime dependency manifest; no committed text inventory or second exact dependency list is permitted.
- `DomainModuleResult` owns one private collection of the closed `DomainLifecycleOutput` enum. Typed producers and exhaustive sinks make a new lifecycle channel fail compilation until every owner is updated; merge and consumption preserve order without a parallel structural scanner.
- Assembly intent and provider facts belong to the Assembly Governance IR and generated plan/catalog gates. Runtime live closure is proved by typed exact-set comparisons over the real wire → validate → compose path.
- `runtime-assembly-residual` remains the single FullOnly owner only for cross-file production risks that Rust visibility and ownership cannot express. It does not snapshot Cargo dependencies, lifecycle fields, or merge implementation shape.
- Dynamic runtime facts that depend on environment, live providers, generated subscriptions, or topology must remain owned by their typed or behavioral carrier, not a static inventory.
- Snapshot, phase, provider, plan closure, event output, listener execution, and launch lifecycle
  facts each have exactly one canonical typed/AST invariant owner. Cross-file Medium carriers must
  use closed typed mutations with focused synthetic reds and real-workspace anti-vacuity; arbitrary
  text needles and parallel gates are forbidden.

## Validation

- Documentation foundation PRs run `cargo xtask verify --fast`.
- Runtime assembly governance PRs additionally run `cargo xtask verify --only runtime-assembly-residual` and `cargo xtask archrules verify`.
- Runtime code PRs run focused crate tests, `cargo test -p runtime` when touching runtime assembly, and `cargo xtask verify --fast`.
- CI/governance PRs update the relevant xtask tests and verify plan membership tests.
