...
- For `profile = "production"`, `assembly validate` must find `run()`-reachable Rust AST evidence for typed profile-specific provider/binding wiring, local JWKS file sources, `rss_access_token_jwks_ready` / `federated_access_token_jwks_ready`, and their managed resources. The exact `listener-pdp` provider receipt must own both the selected JWKS readiness probe and its managed resource (`probes + resources`); a separately staged or merely method-shaped probe call is not lifecycle evidence.
