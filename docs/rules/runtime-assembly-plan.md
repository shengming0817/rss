...
- `RuntimePlanV1` is executable state, not a copy of every declaration. Its private `compile_v1` funnel seeds provider candidates from active manifest declarations and enforces their exact bijection, rejecting missing, duplicate, extra, reordered, or draft provider plans. Executable JSON readers must bind the wire value to the canonical manifest and AssemblyLock and recompile through that funnel; the unbound wire/fingerprint validator returns no executable plan. Draft declarations can remain planning facts in non-production assemblies but can never enter executable provider state; no runtime selector or compatibility fallback exists.
...
- Every direct `assemblies/*` target is classified exactly once in schema-v1 `assemblies/artifacts.toml`; an unknown, omitted, duplicate, or ghost target fails closed.
...
- `ASSEMBLY-ARTIFACT-MATRIX-01` in `xtask/src/assembly_artifacts.rs` is the Medium enforcement carrier. Only its fully checked `VerifiedArtifactMatrix` may feed later deployment/release tooling; `cargo xtask assembly artifacts check` is the focused gate, and typed verify/CI execute it after manifest validation.
- Artifact classification belongs to the `Planned → Deployed` boundary. It is not an input to canonical `AssemblyManifest`, generated modules/providers, AssemblyLock, or RuntimePlan and therefore cannot rotate those identities. Its internal Health inventory is not #1806's authorized public RuntimeInventory, and a supported row is not #1801 production-posture evidence.
