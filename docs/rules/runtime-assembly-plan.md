...
- #1904 adds `deviceidentity` only as a library composition proof. Its canonical manifest is exactly `profile = "demo"`, `domains = ["identity"]`, `topology = "demo"`, with empty listener/framework-contract/workflow-activation sets and the five active persistent pilot roles accepted by the closed registry. The manifest and registry remain the sole provider inventory; this rule does not duplicate their role list. All six DeviceLatent proposal contracts remain draft and unmounted.
