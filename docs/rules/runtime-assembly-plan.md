...
- Assembly manifest v2, AssemblyLock v2, and RuntimePlan v3 are one protocol chain. RuntimePlan v3 adds the closed provider activation value derived from the mandatory `ProviderRole` registry; v2 readers and compatibility dispatch do not exist. The lock binds the canonical manifest to the verified repository definition; `RuntimePlanV3Input::compile_v3` requires workflows and providers (including activation) to exactly match their canonical sources. Executable JSON readers must bind the wire value to the canonical manifest and AssemblyLock and recompile through that funnel.
