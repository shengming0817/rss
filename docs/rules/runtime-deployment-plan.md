...
- Both serving binaries now enter through `prepare_runtime()`, which captures one closed process-level configuration generation before tracing and provider construction. Listener/auth/tracing/serving-OIDC consumers are capability-only; remaining serving migrations are owned by #1784–#1787. RuntimePlan is still summary-oriented, subset assemblies are non-runnable, and no DeploymentPlan/Helm/inventory evidence chain exists.
...
listener address policy, route assembly, and launch accept this capability directly, so an
ambient getter cannot type-check at those production boundaries. When the inbound Internal
...
exactly one capture, `RuntimeInputs` construction, and snapshot-backed Vault/Redis/S3 call, and it
...
binary wrong bindings and early returns, ambient filter restoration, transitive reachable ambient
reader variants, and compliant unrelated-name bait.

Ownership after #1783 is explicit: #1784 covers PG/Redis, #1785 Vault/S3, #1786
event/domain/DLX/worker and composition settings, and #1787 closes the final ambient-read AST
