...
| vault adapter — caller-owned CSR PKI transport（concrete sign + local verify） | `crates/diport` / `adapters/vault` | HashiCorp Vault PKI Sign API（`POST /v1/{mount}/sign/{role}`；`ref: hashicorp/vault api-docs/secret/pki#sign-certificate`）· `jmgilman/vaultrs@0a5643e1dfdb86bbb59542d5490e01862c0e1a27`（`vaultrs/src/api/pki/requests.rs` / `responses.rs`） | RSS 只允许 typed mount/role 的固定 `/sign`，不暴露会返回 private key 的 `/issue`；move-only request 显式携带 common name、按 kind 校验的 SAN、EKU 与相关坐标，响应经 CSR/leaf/chain/expiry 单漏斗验证后才由 Vault adapter 私有铸造 concrete transport evidence。该 evidence 既不能由下游公开构造，也不是 production authorization receipt；出现真实独立 consumer 前不提取共享 provider port。 |
