...
| CI 资源证据 / job-local target / compiler cache writer | 根 CI（`.github/scripts` / reusable workflow / `xtask` 守卫） | `seaweedfs/seaweedfs`（`weed/util/minfreespace.go@5c511c4894c9f6fcbc0e3b7a5d9338628356aeca`；采纳“可用空间低于明确阈值则 fail-fast”，偏离为 CI 关键阶段 `df` 门而非服务持续监控）· `actions/cache`（`restore/action.yml` + `save/action.yml@55cc8345863c7cc4c66a329aec7e433d2d1c52a9`；download/tool采用exact restore，sccache disk snapshot采用受信prefix restore + writer-only唯一key save；完整Cargo target保持job-local）· `mozilla/sccache`（`src/compiler/rust.rs@4fcb1611767704dcbf969ee10a5bdb35ef3eb959`，正式版 `0.15.0`；采纳job-local disk backend、编译结果内容缓存和`CARGO_INCREMENTAL=0`；因该版本原生GHA backend无只读模式，跨job改由Actions restore/save；Rust hasher将绝对`cwd`纳入key，故不承诺RSS crate跨绝对worktree路径命中）· GitHub reusable workflows（官方`reuse-workflows`；采纳薄caller + typed dynamic matrix，完整job单源在`workflow_call`） | `taiki-e/install-action`（钉版release installer；采纳checksum/attestation边界，cached binary仍由repository-owned seal验证）· `Swatinem/rust-cache`（`src/cleanup.ts@c19371144df3bb44fab255c43d04cbc2ab54d1c4`；仅参考关闭incremental的经验，不采用完整target restore/cleanup/save）· `gradle/actions`（`docs/setup-gradle.md@6550634d3eb14a20275549de1588a83267023d42`；采纳cache/build前后诊断与artifact可运维性，RSS使用闭合evidence v4 + 原子append + 5 GiB硬门） |
...
| oidc adapter — 三个互斥 typed token profile 入站验签（RSS/Federated ES256-only；Service HS256-only）+ profile-specific `ManagedResource` | `adapters/oidc` | RFC 8725 §3.11–3.12（显式 typing + 互斥验证规则）· RFC 9068 §2.1（`typ=at+jwt`）· RFC 7515 §4.1.11（`crit`）· `RustCrypto/elliptic-curves`（P-256 ES256）· `RustCrypto/MACs`（HS256） | `maxlambrecht/rust-spiffe` JWT-SVID 验签链范式。RSS **偏离**：sealed profile marker + `OidcProvider<P>`，listener/issuer/audience/key-source 隔离；exact `kid`，无 blind-scan；任何 `crit` 拒绝；16/4/12/1 KiB encoded bounds 先于 decode/JSON/key lookup/crypto/replay；每个 access profile 独占本地 JWKS + readiness，Service 不读 JWKS。 |
| authn — typed token profile 签发（RSS Access ES256、Service Token HS256；Federated 无 issuer） | `authn`（`src/mint.rs`：`JwtIssuer<P>` / `JwtIssuerConfig<P>` / `JwtAccessPrincipal` / `MintedJwt`） | RFC 7515 §7.1（JWS Compact Serialization）· RFC 8725 §3.11–3.12（显式 profile 与互斥 validation）· RFC 9068 §2.1（access token `typ`）· RFC 7519（registered claims） | `Keats/jsonwebtoken` encoding flow。RSS **偏离**：算法、`typ`、RSS 私有 `token_use` 与 maximum lifetime 全由 sealed marker 派生；RSS Access TTL `1..=900s`，Service TTL `1..=300s`；错误 profile 的构造/签发方法在编译期不存在。 |
