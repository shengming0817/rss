...
| authn — JWT 签发（mint/sign）：claims 组装（sub/tenant/kind/exp/iat/iss/aud）+ ES256/HS256 紧凑 JWS 序列化（复用 `diport::Signer` + 注入 `diport::Clock`，纯计算零 crypto/I-O；真实 JWS-兼容 signer adapter〔ES256 定长 r‖s / HS256 HMAC〕+ httpserve 接线留 W） | `authn`（`src/mint.rs`：`JwtIssuer`/`JwtAlg`/`JwtIssueError`/`MintedJwt`） | `RFC 7515 §7.1`（JWS Compact Serialization：`base64url(header)."."base64url(payload)` 为 signing input、签名段 base64url 无填充；authn `JwtIssuer::issue` **直接实现**该组装，与 `adapters/oidc/src/jws.rs` 验签侧**同 RFC 反方向**——verify=解析 / mint=组装）· `RFC 7519`（JWT registered claims：sub/exp/iat/iss/aud 名与语义） | `Keats/jsonwebtoken`（`src/encoding.rs` — `encode<T: Serialize>(&Header, &claims, &EncodingKey) -> String`：serialize header/claims→base64url→拼 signing input→`try_sign`→拼签名段，与 `JwtIssuer::issue` **同形**的工业 mint 流程；RSS **不依赖**——手写 base64+serde_json + 签名委托 `diport::Signer`，守「纯 RustCrypto，不经 jsonwebtoken/ring」纪律）；`JwtAlg` 闭枚举 = OIDC-ALG-WHITELIST-01 的 **mint 侧镜像**（alg=none/RS256 类型层不可 mint） |
