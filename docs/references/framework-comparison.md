...
| CI 资源证据 / 磁盘低水位门 | 根 CI（`.github/scripts` / `xtask` workflow 守卫） | `seaweedfs/seaweedfs`（`weed/util/minfreespace.go@5c511c4894c9f6fcbc0e3b7a5d9338628356aeca`；采纳“可用空间低于明确阈值则 fail-fast”，偏离为 CI 单次 `df` 门而非服务持续 min-free 监控）· `actions/cache`（`action.yml@55cc8345863c7cc4c66a329aec7e433d2d1c52a9`；采纳 restore 主 step / save post-job 生命周期，偏离为不拆分 restore/save action，`before-save` 仅表示 post-job 前最后普通 step） | `gradle/actions`（`docs/setup-gradle.md@6550634d3eb14a20275549de1588a83267023d42`；采纳 CI cache/build 前后诊断与 artifact 可运维性，偏离为 RSS 使用固定 schema v1 JSON + 原子 append + 5 GiB 硬门） |
