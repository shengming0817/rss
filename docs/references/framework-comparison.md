...
|-------------------|-------|----------------------------------|-----------|
| reconcile L4 控制环 | `consistency`（引擎）· `deviceloop`（设备 L4 消费者）· `assemblies/deviceidentity`（#2117 production candidate） | `kube-rs/kube`（`kube-runtime/src/controller/mod.rs@0.99.0`） | `oxidecomputer/omicron`。RSS #2117 以 generated RuntimePlan/lock/provider catalog、supervised worker/listener 和 bounded drain 物化 candidate；六契约仍 Draft，且不声明 T3/profile activation。 |
| SPIFFE workload identity / External CSR | `adapters/httpd` · `assemblies/deviceidentity` | `maxlambrecht/rust-spiffe`（`spiffe/src/workload_api/x509_source.rs@spiffe-0.16.0`） | RSS 只用 X509Source 构造 exact local/remote SPIFFE mTLS，不提供 token/plaintext fallback；CSR resolver 是只读 evidence lookup。 |
| Vault PKI CSR signing | `adapters/vault` · `composition/identity` | `jmgilman/vaultrs`（`src/api/pki/requests.rs@0a564` 的 sign request） | RSS 保留 pre-dispatch unavailable 与 post-dispatch outcome-unknown quarantine 的闭值区分，并由当前 acquisition 在本地绑定 CN/SAN/EKU/validity/receipt。 |
| background lifecycle ownership | `diport` / `bootstrap` / `eventexec` / `runtimeexec` | `tokio-rs/tokio`（`tokio/src/runtime/task/join.rs`：raw `JoinHandle` drop detaches）· `tokio-rs/axum`（graceful shutdown） | RSS 明确偏离 raw-handle owner：Tokio task 统一由 move-only `ManagedTask` 持有并发布 closed terminal state；dedicated thread 由 `ManagedBlockingWorker`；broker settlement 由 consumer；listener failure/root cancellation/总预算由 RuntimeExec。 |
| saga L3 编排 | `consistency` / `eventexec` | `oxidecomputer/steno`（`src/lib.rs`） | `temporalio/sdk-rust`（`crates/sdk-core/src/lib.rs`） |
| Postgres LocalTx transaction runner / TLS trust store | `adapters/postgres` | `launchbadge/sqlx`（`sqlx-core/src/transaction.rs`，消费式 commit/rollback + Drop rollback safety net；`sqlx-core/src/pool/connection.rs`，`close_on_drop` 隔离 pooled connection；`sqlx-core/src/net/tls/tls_rustls.rs@bab1b022bd56a64f9a08b46b36b97c5cff19d77e`） | `launchbadge/sqlx`（`sqlx-postgres/src/transaction.rs`，Postgres BEGIN/COMMIT/ROLLBACK manager）。RSS #1954 明确偏离 SQLx 0.8.6 的“公共根 + 显式根 overlay”：生产 `PgPrivateCa` 走受控 downstream core patch，以空 store + inline PEM 实现独占根；无显式根的非生产 SQLx 消费仍保持上游 WebPKI 行为。 |
...
| vault adapter — caller-owned CSR PKI transport + receipt-bound production artifact bridge | `crates/diport` / `crates/identity` / `adapters/vault` | HashiCorp Vault PKI Sign API（`POST /v1/{mount}/sign/{role}`；`ref: hashicorp/vault api-docs/secret/pki#sign-certificate`）· `jmgilman/vaultrs@0a5643e1dfdb86bbb59542d5490e01862c0e1a27`（`vaultrs/src/api/pki/requests.rs` / `responses.rs`）· `instant-labs/instant-acme src/order.rs@8e4441f`（CSR/order artifact 消费边界） | RSS 只允许 typed mount/role 的固定 `/sign`，不暴露会返回 private key 的 `/issue`；move-only request 显式携带 durable receipt correlation、从 authenticated device scope 派生的 common name、按 kind 校验的 SAN、EKU 与相关坐标，响应经 CSR/leaf/chain/expiry 单漏斗验证。`VaultPkiArtifactEvidence` 仍不是 authorization；只有 production join 消费 provider config identity 相同的 official Vault closure/evidence 与当前 desired acquisition/exact receipt 才可 mint production artifact。 |
