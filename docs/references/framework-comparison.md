...
| Cargo workspace package graph / target facts | `crates/workspacefacts`（`xtask` command-scope loader） | `guppy-rs/guppy`（`guppy/src/graph/graph_impl.rs`、`build_targets.rs`、`query.rs` @ 0.17.26；完整 metadata JSON → `PackageGraph`、reverse query、build target catalog） | RSS 仅保留 owned package/path/target 薄适配；不自建 metadata schema、reverse adjacency、BFS、feature/platform graph，也不使用 guppy `MetadataCommand` 绕过 xtask command funnel |
...
| CI 固定 Job 委托 / result-only 聚合 | 根 CI（reusable workflow / `xtask` typed selector、executor 与 gate） | GitHub reusable workflows（官方 `reuse-workflows`；采纳薄 caller、闭合 typed input 与固定 `workflow_call` Job 实现）· GitHub Actions job result context（固定 `selector`、`check`、`test-affected`、`integration-critical` 的最终状态由 result-only gate 聚合；LocalOnly required evidence 由 `test-affected` producer 拥有） | cargo-nextest（组件测试选择、JUnit 与 replay sidecar 仅作执行/诊断；不得作为第二个 gate verdict owner） |
...
| feature flag 求值（constraints + percentage stickiness + decision reason/stale） | `settings` | `Unleash/unleash-types-rs`（`src/client_features.rs`：Constraint/Operator/`ClientFeature.enabled|stale|variants`） | `Unleash/unleash-client-node`（`src/variant.ts`）；RSS 偏离：reason 闭集自研、`stale`=同步陈旧非 lifecycle、`variant` 未建模 |
