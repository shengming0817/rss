...
- **server policy capability 同样必填**：HTTP adapter 只能消费 `AuthenticatedRoutes::into_server_service(ServerRequestBudget)`
  返回的字段私有 `ServerService`；该类型不实现 axum make-service，实际 bind 只能由 `httpd` 私有 wrapper
