...
- **日期**：2026-07-02
- **关联**：Feature #1576 · PBI #1587 · ADR-006（内置 typed authplan / OPA 取舍）· `docs/rules/tenancy.md`
...
| PostgreSQL RLS | SQL policy on tables | database policy evaluation during query execution | table rows + session GUC via `current_setting` | FORCE RLS + `SET LOCAL rss.tenant_id` + non-bypass role | row filtering / write check only; no route or field obligation | database role, statement, and durable app audit provide evidence | `schema-rls`, `setlocal-funnel`, `pg-tenant-tx-guard`, startup `verify_rls_capability()` | strongest data boundary, but cannot decide route permission or field projection | adopt as tenant data boundary; keep separate from route Authorizer |
...
- The same gate requires `docs/rules/tenancy.md` and ADR-006 to link this ADR.
