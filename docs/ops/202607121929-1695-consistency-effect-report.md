...
JSON 只输出 `schemaVersion = 2`，不保留 v1 alias 或双 schema。v2 在静态 posture 之外增加
`localOnlyReceiptCoverage` 和每条 contract 的显式 `sourceReceiptRegistration`
（`evidence=sourceRegistered`、`enforcement=reportOnly`、`status=registered|missing|notApplicable`）；
旧消费者必须直接升级解析 v2。

## 检查规则

- `status = "passed"`：production mount 与全部 LocalOnly static proof 无 finding。
...
- `localOnlyReceiptCoverage.enforcement = "reportOnly"`，其 `registered|missing` 表示 canonical receipt
  source site 是否登记，不表示本次命令执行过 runtime test。`missing` 不改变顶层静态 `status`。
