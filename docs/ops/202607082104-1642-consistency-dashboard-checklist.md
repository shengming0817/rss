...
|---|---|---|---|
| Outbox oldest pending age | `outbox_oldest_pending_age_seconds` | none | `OutboxBacklogOldestAgeHigh`; check relay / broker / DB |
| Outbox pending depth | `outbox_pending_depth` | none | `OutboxPendingDepthHigh`; check throughput and relay batch |
| Outbox partition blocked | `outbox_partition_blocked_depth` | none | `OutboxPartitionBlocked`; inspect DLX head, no partition key in metric |
...
| Outbox publish disposition | `sum by (status) (rate(outbox_publish_total[5m]))` | `status` | Requeue storm / reject path diagnosis |
| Outbox DLX rate | `rate(outbox_publish_total{status="reject"}[5m])` | `status` | `OutboxDlxGrowth`; identify the row from durable DLQ inventory |
...
| Consumer settle outcome | `sum by (action, outcome) (rate(consumer_settle_total[5m]))` | `action`, `outcome` | Broker settle failures and reject/requeue mix |
| Consumer DLX write | `sum by (outcome) (increase(consumer_dlx_write_total[5m]))` | `outcome` | `ConsumerDlxWriteError`; DLX audit write failed |
| Consumer release failed | `increase(consumer_release_failed_total[5m])` | none | `ConsumerReleaseFailed`; claim release failed after DLX failure |
| Consumer lease lost | `rate(consumer_lease_lost_total[5m])` | none | `ConsumerLeaseLostHigh`; check handler duration and lease TTL |
