...
Primary 流量和 kubelet HTTP probe 都不能直接访问 Pod IP 上的 settingsonly loopback socket：两者必须经同 Pod
sidecar 的 TLS/探针端口转发；也可让 sidecar 在同一网络命名空间内执行 loopback probe 后导出自身的 kubelet 探针端口。
文件/Secret generation 与 image 必须原子发布；不做旧 schema 双读或别名回退。
...
只有 `/readyz` 返回 200 后才能接流量。Primary 为配置的 loopback端口，Health 为独立 loopback 端口。发送
