#!/bin/sh
# Production preflight for migration 0069. Run on the primary PostgreSQL host
# after old writers stop and before the singleton migration bootstrap starts.
...
case "$MAINTENANCE_WINDOW_SECONDS" in
  ''|*[!0-9]*) echo "MAINTENANCE_WINDOW_SECONDS must be a non-negative integer" >&2; exit 2 ;;
esac

GIB=1073741824
CREDENTIAL_ROW_LIMIT=50000000
CREDENTIAL_BYTES_LIMIT=$((10 * GIB))
DATA_BUDGET=$((12 * GIB))
WAL_BUDGET=$((20 * GIB))
ARCHIVE_BUDGET=$((20 * GIB))
RESERVE=$((5 * GIB))
REPLICA_LAG_LIMIT=$((256 * 1024 * 1024))
# 0069 has a five-minute statement timeout. Preserve three minutes for a
# failed transaction to roll back and for the operator to make a safe decision.
MINIMUM_WINDOW_SECONDS=480

if [ "$MAINTENANCE_WINDOW_SECONDS" -lt "$MINIMUM_WINDOW_SECONDS" ]; then
  echo "maintenance window $MAINTENANCE_WINDOW_SECONDS seconds is below required $MINIMUM_WINDOW_SECONDS" >&2
  exit 1
fi
...
credential_rows=$(pg "SELECT count(*) FROM public.credentials")
if [ "$credential_rows" -gt "$CREDENTIAL_ROW_LIMIT" ]; then
  echo "credential rows $credential_rows exceeds rollout limit $CREDENTIAL_ROW_LIMIT" >&2
  exit 1
fi
credential_bytes=$(pg "SELECT pg_total_relation_size('public.credentials'::regclass)")
if [ "$credential_bytes" -gt "$CREDENTIAL_BYTES_LIMIT" ]; then
  echo "credential bytes $credential_bytes exceeds rollout limit $CREDENTIAL_BYTES_LIMIT" >&2
  exit 1
fi
echo "credential_rows=$credential_rows limit_rows=$CREDENTIAL_ROW_LIMIT"
echo "credential_bytes=$credential_bytes limit_bytes=$CREDENTIAL_BYTES_LIMIT"
...
case "$archive_mode" in
  on|always) ;;
  *) echo "archive_mode=$archive_mode; production 0069 rollout requires WAL archive" >&2; exit 1 ;;
esac
...
if [ -n "$archive_dir" ]; then
  ...
  case "$remote_archive_free" in
    ''|*[!0-9]*) echo "set WAL_ARCHIVE_DIR or provider-derived REMOTE_ARCHIVE_FREE_BYTES" >&2; exit 2 ;;
  esac
...
pg "SELECT pg_logical_emit_message(false, 'rss.0069-capacity-gate', 'archive-probe')" >/dev/null
...
while [ "$attempt" -lt 60 ]; do
  ...
  if archive_target_present; then
    ...
    echo "maintenance_window_seconds=$MAINTENANCE_WINDOW_SECONDS minimum_seconds=$MINIMUM_WINDOW_SECONDS"
    echo "0069 account-security capacity gate: PASS"
    exit 0
  fi
  attempt=$((attempt + 1))
  sleep 1
