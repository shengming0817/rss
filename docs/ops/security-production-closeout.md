...
| Egress TLS (AMQP/Redis/S3/PG) | **Generic closeout scope = `assemblies/runtime` only** (`profile=production`). Runtime egress always Deny + VerifyFull. `RSS_AMQP_ALLOW_PLAINTEXT` / `RSS_REDIS_ALLOW_PLAINTEXT` / `RSS_S3_ALLOW_PLAINTEXT` / `RSS_PG_SSL_MODE` are banned (fail-fast if present). Required private CA paths: `RSS_AMQP_CA_CERT_PEM_PATH`, `RSS_REDIS_CA_CERT_PEM_PATH`, `RSS_S3_CA_CERT_PEM_PATH`, `RSS_PG_SSL_ROOT_CERT_PATH`. Bundle URLs must use `amqps://` / `rediss://` / `https://`. PG additionally snapshots its PEM into mandatory `PgPrivateCa`; the patched SQLx store begins empty, so public WebPKI roots are excluded. `settingsonly` and `identityaudit` also require the PG CA and reject the removed `sslMode` field. Ingress-only exception: `RSS_LISTENER_ALLOW_PLAINTEXT=dev-container` for compose port mapping (#1649). | `cargo xtask assembly validate` requires the runtime `PgPrivateCa` funnel; Draft-07 schemas reject missing/legacy PG TLS fields; the vendored core T1 test proves exact store initialization; shipped-feature guard rejects `postgres/test-support` in production graphs; real PG TLS tests cover fixed runtime roles. |
