...
| Egress TLS (AMQP/Redis/S3/PG) | **Generic closeout scope = `assemblies/runtime` only** (`profile=production`). Runtime egress always Deny + VerifyFull. `RSS_AMQP_ALLOW_PLAINTEXT` / `RSS_REDIS_ALLOW_PLAINTEXT` / `RSS_S3_ALLOW_PLAINTEXT` / `RSS_PG_SSL_MODE` are banned (fail-fast if present). Required private CA paths: `RSS_AMQP_CA_CERT_PEM_PATH`, `RSS_REDIS_CA_CERT_PEM_PATH`, `RSS_S3_CA_CERT_PEM_PATH`, `RSS_PG_SSL_ROOT_CERT_PATH`. Bundle URLs must use `amqps://` / `rediss://` / `https://`. Ingress-only exception: `RSS_LISTENER_ALLOW_PLAINTEXT=dev-container` for compose port mapping (#1649). The `identityaudit` reference assembly independently closes AMQP/Redis through its Draft-07 schema and the adapter production API, which exposes only explicit private-CA constructors. | `cargo xtask assembly validate` keeps the generic egress-tls AST gate on `manifest.name == "runtime"`; identityaudit rejects missing CA paths and non-TLS URLs, while shipped-feature and event-transport guards reject test-only WebPKI/raw-pool paths in production graphs. |
