...
| P1-11 | **签名游标 tamper-proof cursor**（HMAC-SHA256 keyset 游标 + 强制 scope(排序列 hash)+context(查询指纹) + ValidateCursorScope + current/previous 双 key rotation + MaxCursorTokenBytes 防 DoS） | query/cursor.go | `rss-contract::PageCursor` 已提供 4096-byte bound、canonical base64url 与 stale 分类，但按 Foundation 边界不拥有 HMAC/scope/context/rotation；tenancy.md 无游标↔行权限绑定 | 在实际分页状态 owner 增加 consumer-specific `PageCursor` codec（HMAC-SHA256+强制 scope/context），HMAC 复用 primitives::crypto MacVerifier；不得把 provider 形状上推 Foundation | 多租户下未签名游标可伪造越 sort/scope=IDOR/scope-escape |
