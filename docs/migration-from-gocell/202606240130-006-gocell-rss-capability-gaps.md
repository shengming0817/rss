...
| P1-9 | **Vault Transit 信封加密 KeyProvider + AEAD 显式 AAD 绑定 + config value 加密**（provider-可换 Encrypt/Decrypt DI port + 本地 AES-GCM DEK/EDK + AAD=cell/tenant/key 复合绑定防跨 entry/跨租重用 + key-id rotation 常数时间匹配）。合并 内核域 #4/#7、adapter域 #1、域Cell #10 | vault/transit_provider.go + aeadutil/gcm.go + verifykeyid.go + configcore/crypto/aad.go | diport 无 KeyProvider（7 ports 无此）；secure/aead.rs Aead trait `seal(plaintext)`/`open(ct)` **无 aad 参数**；primitives/crypto.rs doc 指向 diport::KeyProvider 但 diport 无此 port=doc 落空；settings ConfigValue 仍明文 newtype | `diport` 新增 KeyProvider DI port（dynosaur Send 变体）；`adapters/vault` 实现 Transit 信封加密（本地 AES-GCM DEK + EDK vN 格式 + AAD 强绑定）；`secure::Aead` seal/open 增 aad 参数；`settings` ConfigValue 加密路径 + AADForConfig；KeyId::parse+match_key_id 经 constant_time_eq | 静态数据无加密；AAD 缺失=密文可跨 tenant/跨 key 重放（安全语义削减） |
...
| **consistency** | projection Apply system 身份显式注入入口（P1-5）；reconcile Loop chokepoint install system identity + strip ambient（P1-6）；FencedWriter sealed 单一写面 funnel Hard 化（P0-9a）；SagaTerminalStatus 闭值枚举（P2-1）；projection Phase 枚举 + rebuild 编排（P2-2）；FailOpenTracker + IdempotencyStore.extend(ttl)（P2-5） |
