...
Command authoring reads this exact R/G pair from `DesiredStateSnapshot`; callers cannot provide the receipt independently.
The PostgreSQL enqueue transaction exact-checks tenant, device, generation, receipt, policy hash, artifact coordinates and
canonical intent before any command, action, journal or outbox write.
...
32-byte semantic intent digest, desired authorization receipt and generation, positive fence epoch, deadline, checked
...
An application receipt is a publishable fact produced from the durable ingress transaction outcome. Committed,
duplicate, and stale variants carry the server-restored desired authorization receipt and generation; rejected variants
cannot carry either field. RSS restores lineage through the authenticated tenant/device/generation database join—never
from ACK/report bodies. Missing lineage or store failure rolls back a lineaged outcome; an unknown generation becomes the
non-oracle `rejected/NotAccepted` variant. Publication occurs only after the transaction commits. A transport
acknowledgement or an in-memory handler result cannot create this fact.
...
The sealed internal receipt makes `CertScope`, `CertSerial`, and `CertNotAfter` available to RSS readiness and revocation checks without making them caller-forgeable command fields. The artifact is issued/resolved for one command-authoring decision. Device commands carry the server-derived authorization receipt/generation lineage together with fence, policy, intent, and public artifact ID/digest coordinates; they never carry a private key, raw CSR, serial authorization, or other material not authorized by this capability. A deterministic simulator may return a non-production artifact type, but that type can satisfy neither `AuthorizedCertificateArtifact` nor `ExternalPkiProviderClosure` production dependencies.
