...
|---|---|---|---|
| **httpserve** | typed route funnel（**ADR-009**，#1113/#1103/#1690）：`Listener` sealed marker + `ListenerRouter<L>::mount`（只收 `GeneratedEndpoint` / `GeneratedPrimaryEndpoint`）+ 闭值状态机 `UnfinalizedRoutes`→auth finalizer→不可 bind 的 `AuthenticatedRoutes`→业务 `RateLimitedRoutes` / Health `HealthRoutes`；`RouteMeta` 原样持 `HttpRouteEvidence`；Health 固定 builder | route 注册=`route_group::<L>` register 闭包 `FnOnce(ListenerRouter<L>)->Result<..>`；裸 `axum::Router` / `MethodRouter` / route 字段不出 production public API | ref: tower/axum（`Router<S>` 状态类型） |
