...
- `contract.toml`：`id="<domain>.<command>"`、`kind="command"`、`consistencyLevel="OutboxFact"`（L2，**机器锁定**：R15 `CommandConsistency` 强制 `kind=command ⇒ OutboxFact`，误标 L0/L1/L3/L4 即 verify 红，见 `contracts/**/contract.toml`、`generated` 与 `crates/consistency`）、`owner`、`topic="<domain>.commands.<name>"`（R8：`lifecycle=active` 时 **必填**；R9 `PerKindFieldScope` 允许 event ∪ command 使用 `topic`，出现在 command 是合法的，不会被拒）。runtime consumer/producer 的路由 key 从此 `topic` 读取，经 codegen 烤入 wrapper 常量，不在运行期重新派生。
...
**治理**（xtask，SAGA-CONTRACT-01，对齐 generated / diport::SagaDurableStore / saga conformance）：
