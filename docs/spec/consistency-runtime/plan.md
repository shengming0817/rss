...
| DLX payload confidentiality and operator boundary | `docs/rules/eventbus.md` §DLX 与幂等, `docs/rules/tenancy.md` §持久化模式 tenant 作用域合约 | **Hard/Medium**: durable runtime requires `RSS_DLX_PAYLOAD_KEY_NAME` and Vault transit configuration; plaintext `{"bytes":[...]}` DLX payload shape is forbidden. Each of list/inspect/replay/redrive/resolve consumes a move-only `DlqOperatorAuthorization<A>` that binds action, tenant, caller, verified operator subject and durable start audit ID; only `runtime` can mint through the isolated `dlqauthmint` dependency boundary. Dylint retains only the runtime sequencing guard. The pre-auth start row uses an identity-neutral attempt actor; the finish row records the verified subject, and both share audit ID, tenant and action family. Replay/redrive/resolve return a typed durable receipt only after mutation and finish audit commit in one tenant transaction; audit failure rolls back the mutation. Replay additionally requires typed `DeadLetterId`, a caller-supplied new `IdemKey`, and the same `KeyProvider`. | Durable audit persistence and the exact runtime order are Medium fail-closed guarantees; cross-crate mint authority, action/tenant binding and proof non-forgeability are Hard. Tenant dead-letter writes remain inside the tenant-scoped PG funnel. |
