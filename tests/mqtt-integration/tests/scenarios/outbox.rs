...
async fn provision(owner: &sqlx::PgPool) -> anyhow::Result<()> {
    ...
    sqlx::raw_sql(rss_transactional_messaging_postgres::MIGRATION_SQL)
        .execute(owner)
        .await?;
    sqlx::raw_sql("GRANT USAGE ON SCHEMA rss_transactional_messaging TO mqtt_runtime; GRANT SELECT ON rss_transactional_messaging.policy TO mqtt_runtime; GRANT SELECT,INSERT,UPDATE,DELETE ON rss_transactional_messaging.inbox TO mqtt_runtime; GRANT SELECT ON rss_transactional_messaging.outbox TO mqtt_runtime; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.prepare_outbox_partitions(jsonb),rss_transactional_messaging.append_outbox(text,text,text,jsonb,bytea) TO mqtt_runtime; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.claim_outbox(uuid,text,integer,bigint),rss_transactional_messaging.outbox_lease(uuid,bigint,uuid,bigint,bigint,uuid),rss_transactional_messaging.settle_outbox(uuid,bigint,uuid,bigint,text,uuid) TO mqtt_runtime; CREATE TABLE public.mqtt_handoff (message_id text PRIMARY KEY, payload bytea NOT NULL);").execute(owner).await?;
    Ok(())
}
