...
async fn setup(fixture: &testkit::PgTlsFixture) -> anyhow::Result<Fixture> {
    ...
    sqlx::raw_sql("GRANT USAGE ON SCHEMA rss_transactional_messaging,rss_device_command TO device_runtime; GRANT SELECT ON rss_transactional_messaging.policy TO device_runtime; GRANT SELECT,INSERT,UPDATE,DELETE ON rss_transactional_messaging.inbox TO device_runtime; GRANT SELECT ON rss_transactional_messaging.outbox TO device_runtime; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.prepare_outbox_partitions(jsonb),rss_transactional_messaging.append_outbox(text,text,text,jsonb,bytea) TO device_runtime; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.claim_outbox(uuid,text,integer,bigint),rss_transactional_messaging.outbox_lease(uuid,bigint,uuid,bigint,bigint,uuid),rss_transactional_messaging.settle_outbox(uuid,bigint,uuid,bigint,text,uuid) TO device_runtime; GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA rss_device_command TO device_runtime; GRANT SELECT ON ALL TABLES IN SCHEMA rss_device_command TO device_runtime;").execute(&owner).await?;
