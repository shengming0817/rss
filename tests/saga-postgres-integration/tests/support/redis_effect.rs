...
impl RedisSagaEffectFixture {
    pub(crate) fn new(store: Pool) -> Self {
        Self {
            store,
    ...
    pub async fn apply(
        ...
        let mut conn = self.store.get().await.map_err(provider_error)?;
...
fn storage_key(key: &EffectKey) -> String {
...
where
    E: std::error::Error,
{
    let _ = error; // Provider diagnostics remain opaque in this private test fixture.
