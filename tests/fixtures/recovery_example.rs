...
impl Fixture {
    pub async fn new(network: &testkit::BridgeNetwork) -> anyhow::Result<Self> {
        ...
        sqlx::raw_sql("GRANT USAGE ON SCHEMA rss_transactional_messaging TO recovery_operator; GRANT SELECT ON rss_transactional_messaging.policy TO recovery_operator; GRANT SELECT,INSERT,UPDATE,DELETE ON rss_transactional_messaging.inbox TO recovery_operator; GRANT SELECT,INSERT,UPDATE ON rss_transactional_messaging.outbox TO recovery_operator; GRANT USAGE ON ALL SEQUENCES IN SCHEMA rss_transactional_messaging TO recovery_operator; GRANT SELECT,INSERT,UPDATE(recovery_version) ON rss_transactional_messaging.consumer_dead_letter TO recovery_operator; GRANT SELECT,INSERT ON rss_transactional_messaging.recovery_operations TO recovery_operator; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.claim_outbox(uuid,text,integer,bigint),rss_transactional_messaging.outbox_lease(uuid,bigint,uuid,bigint,bigint,uuid),rss_transactional_messaging.settle_outbox(uuid,bigint,uuid,bigint,text,uuid) TO recovery_operator;").execute(&admin).await?;
