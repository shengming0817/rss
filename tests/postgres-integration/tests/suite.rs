...
async fn postgres_transactional_messaging_suite() -> anyhow::Result<()> {
    tokio::time::timeout(Duration::from_secs(180), async {
        ...
        sqlx::raw_sql("GRANT USAGE ON SCHEMA rss_transactional_messaging TO tmsg_runtime; GRANT SELECT ON rss_transactional_messaging.policy TO tmsg_runtime; GRANT SELECT,INSERT,UPDATE,DELETE ON rss_transactional_messaging.inbox TO tmsg_runtime; GRANT SELECT ON rss_transactional_messaging.outbox TO tmsg_runtime; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.prepare_outbox_partitions(jsonb),rss_transactional_messaging.append_outbox(text,text,text,jsonb,bytea) TO tmsg_runtime; GRANT EXECUTE ON FUNCTION rss_transactional_messaging.claim_outbox(uuid,text,integer,bigint),rss_transactional_messaging.outbox_lease(uuid,bigint,uuid,bigint,bigint,uuid),rss_transactional_messaging.settle_outbox(uuid,bigint,uuid,bigint,text,uuid) TO tmsg_runtime;")
            .execute(&owner).await?;
        ...
        Box::pin(partition::run(runtime.clone(), &owner, &raw_runtime, config.clone())).await?;
