...
//!
//! INVARIANT: DIPORT-ENVELOPE-WIRE-WRITER-01
//!
...
//! - callsite 放行（与 `rss_diport_impl_allowlist` 完全相同逻辑，原样复用）：① `LOCAL_CRATE` 名 ==
//!   "diport"（定义方内部调用合法）；② `CARGO_MANIFEST_DIR` 父目录名 ∈ adapters/bins/assemblies。
//!   键 package 位置而非源文件路径 ⇒ 域 crate 把调用藏进 `src/adapters/` 子目录无法绕过（manifest
//!   dir 仍 `crates/<domain>`，父目录 `crates`）。
//!
...
//! 非 allowlist，红例 `insert_wire_pair` 调用**必报** 1 条）；绿向（恒报 / 误伤 adapter）由
//! `cargo xtask verify` 的 `cargo dylint --all` 工作区跑锁（adapter 真实调用 0 诊断）——是 verify 机器门。
//!
//! Hard 化评估（ai-robust.md §审查要求）：无低成本 Hard 路径。`insert_wire_pair` 必须 `pub`（跨 crate
//! adapter 须调），无法用可见性封闭；sealed-trait 无法跨 crate 限制调用者；AST lint 是最强可用载体。
...
dylint_linting::declare_late_lint! {
    /// ### What it does
    /// 标记**非** callsite-allowlist crate（package manifest 父目录不在 `adapters`/`bins`/`assemblies`，
    /// 即域 / 服务 / 引擎 / 基础 crate 等）里对 `diport::EnvelopeMetadata::insert_wire_pair` 的调用。
    ///
    /// ### Why is this bad?
    ...
    Warn,
    "reserved-capable wire 写面 EnvelopeMetadata::insert_wire_pair 仅 adapter（adapters/）/ 组合根（bins/assemblies）可调（callsite allowlist，INVARIANT DIPORT-ENVELOPE-WIRE-WRITER-01）"
}
...
impl<'tcx> LateLintPass<'tcx> for RssDiportEnvelopeReservedWriter {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        if segment.ident.as_str() != "insert_wire_pair" {
            return;
        }
        ...
        // 双重确认：item 名确为 insert_wire_pair（belt-and-suspenders）。
        if cx.tcx.item_name(method_did).as_str() != "insert_wire_pair" {
            return;
        }
...
fn ui() {
    dylint_testing::ui_test_example(
        env!("CARGO_PKG_NAME"),
        "rss_diport_envelope_reserved_writer_ui",
    );
}
