...
//! `rss_diport_impl_allowlist` — RSS 治理 dylint lint：`diport` provider port trait 仅 blessed
//! adapter / 组合根可 impl（impl-site allowlist）。
//!
//! INVARIANT: DIPORT-IMPL-ALLOWLIST-01 { level = "Medium", exec = "check", source = "dylint" }
//!
//! Provider port trait（`Signer`/`Publisher`/`Subscriber`/`AuditSink` 的 trait_variant Send 变体 +
//! 基 trait `*Local` + sync `Clock`/`SubscribeInitializer`）集中在 `diport`。ADR-003 §4.2
...
//!
//! 判定面：
//! - trait 归属：被 impl 的 trait 其 `DefId` crate 名 == `diport`，且完整 DefPath 不是 lifecycle seam 的
//!   `diport::managed_resource::{ManagedResource, ManagedResourceLocal}`。其余 trait fail-closed 纳入限制，
//!   新增 provider port 自动受守；完整 DefPath 避免同名 trait 误豁免。
...
//!   成员目录 `adapters` / `bins`（对齐 `xtask/src/layers.rs` 顶层成员分层）——新增 adapter
...
//! 不进生产构建，无生产绕过口）；③ allowlist 顶层成员目录集（`adapters`/`bins`）扩项无机器复核
//! （与 `layers.rs` 顶层成员约定同源，靠 greppable + 治理评审）。键 `CARGO_MANIFEST_DIR` 父目录而非源文件
//! 路径，无「祖先目录同名误放行」「域内子目录绕过」隐患。
//!
//! anti-vacuity（守卫非恒真 / 恒假，两向均机器锁）：红向由 UI golden 锁（非 allowlist example 中
//! `Clock` provider port **必报 1 条**）；lifecycle 绿向由同一 UI 中 `ManagedResource` +
...
const LIFECYCLE_TRAIT_PATHS: [&[&str]; 2] = [
    &["diport", "managed_resource", "ManagedResource"],
    &["diport", "managed_resource", "ManagedResourceLocal"],
];

dylint_linting::declare_late_lint! {
    /// ### What it does
    /// 标记**非** allowlist crate（package manifest 父目录不在 `adapters`/`bins`，即域 / 服务 /
    /// 引擎 / 基础 crate）里对 `diport` 定义的 provider port trait 的 `impl`。
    ///
    /// ### Why is this bad?
    /// Provider port（`Signer`/`Publisher`…）的正确性要求 provider 可互换、impl 仅限 blessed
    ...
    /// 默认不被扫（test mock impl 放行）；allowlist 顶层成员目录集（`adapters`/`bins`）扩项无机器
    ...
    Warn,
    "diport provider port trait 仅 adapter / 组合根可 impl（impl-site allowlist，INVARIANT DIPORT-IMPL-ALLOWLIST-01）"
}
...
impl<'tcx> LateLintPass<'tcx> for RssDiportImplAllowlist {
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        ...
        // 仅限制 diport provider port；ManagedResource 两个 trait_variant 身份是 lifecycle seam。
        if !trait_requires_impl_allowlist(cx, trait_did) {
            return;
        }
        ...
        span_lint_hir_and_then(
            ...
            item.span,
            format!(
                "diport provider port trait `{}` 仅 adapter / 组合根可 impl：此 crate 不在 impl-site allowlist",
                cx.tcx.item_name(trait_did)
            ),
            |diag| {
                diag.help(
                    "把 provider 实现放到 `adapters/<name>` 或 `bins/`，域 / 服务 crate 经构造器注入 `Box<DynX>` / `Arc<DynX>` 消费；确需在 allowlist 外 impl，在该 impl 块加 `#[allow(rss_diport_impl_allowlist)] // reason: ...`（item-level 逃生门），并在 PR review 说明理由",
                );
            },
        );
    }
...
/// `diport` 中除两个 lifecycle trait 外的 trait fail-closed 受 provider impl-site allowlist 限制。
/// crate identity 防止本地 `mod diport` 伪造；完整 DefPath 防止其它模块的同名 trait 被误豁免。
fn trait_requires_impl_allowlist(cx: &LateContext<'_>, trait_did: DefId) -> bool {
    if cx.tcx.crate_name(trait_did.krate).as_str() != "diport" {
        return false;
    }
    let path = cx.get_def_path(trait_did);
    !LIFECYCLE_TRAIT_PATHS.iter().any(|expected| {
        path.iter()
...
///    不变）其**父目录名** ∈ workspace 顶层成员目录 `adapters` / `bins`。键 package 位置而非
