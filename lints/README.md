...
| `rss_crosstenant_callsite` | TENANCY-CROSSTENANT-CAP-01 | vocab 跨租户 All-scope mint 三步（`CrossTenantCapability::issue_for_verified_super_admin()` / `CrossTenantVisibility::authorize()` / `RowVisibility::new_cross_tenant()`）仅 `authn` crate 可调用（跨租户 capability 签发 callsite-allowlist；funnel 下游约束——上游私有 `_seal` 字段在 vocab 是 Hard，本 lint 守下游「谁可调」为 Medium）。默认 `Warn`。捕获对这些 assoc fn 的**任意 path 引用**（直接 call callee / 函数项别名 / fn-pointer 强转，凡解析到目标 `DefId` 即报，杜绝「先别名再调用」绕过）；仍 intraprocedural（allowlist crate 内跨函数 wrapper 洗白不追，#1085）。UI 用两个 example target（`crosstenant_callsite_ui` 红 / `authn` 绿）证 allowlist 分支。 |
| `rss_dlq_operator_callsite` | EVENTBUS-DLQ-OPERATOR-CAP-01 | `eventexec::OperatorDlqCapability::issue_for_authorized_operator()` 仅 admin/PDP 边界 crate（当前 `httpserve`）可调用（DLQ replay/redrive mutation capability 签发 callsite-allowlist；上游私有 `_seal` 字段为 Hard，本 lint 守下游「谁可调」为 Medium）。默认 `Warn`。捕获直接 call 与函数项别名；`#[cfg(test)]` 子树默认不扫，测试 fixtures 可直接 mint。 |
...
| `rss_handler_local_principal_authz` | HANDLER-LOCAL-PRINCIPAL-AUTHZ-01 | handler/domain 禁用 `Authenticated` / `Principal` 上的 role/self 本地授权分支，必须消费 route gate 插入的 `AuthorizedSubject`。默认 `Warn`。 |
