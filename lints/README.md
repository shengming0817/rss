...
| `rss_dlq_operator_callsite` | EVENTBUS-DLQ-OPERATOR-CAP-01 | `eventexec` operator capability 的 `issue_for_authorized_operator()` 仅 admin/PDP 边界 crate（当前 `httpserve`）或 runtime `operator::{dlq,reconcile}` 的精确 nested wrapper 可调用；私有字段 `AuthorizedDlqOperatorReceipt` 的 public constructor 更严格地只允许 `runtime::operator::dlq::dlq_operator_receipt`，`VerifiedOperatorSubject` 只能消费该 receipt。私有字段闭合 struct literal，constructor exact-callsite gate 为 Medium。默认 `Warn`。捕获直接 call 与函数项别名；`#[cfg(test)]` 子树默认不扫，测试 fixtures 可直接 mint。 |
