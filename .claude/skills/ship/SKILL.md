...
4. **本地验证（label 后执行）**：运行 `make -C <worktree> ci CI_BASE=<remote>/develop`，其中 `<worktree>` 是阶段 3 记录的绝对路径，展开后的命令形如 `make -C /absolute/worktrees/<type>/<name> ci CI_BASE=<remote>/develop`。验证政策遵循[验证规则](../../../docs/rules/verification-scope.md)。失败则回到相应实施批次，修复、推送并重新完成收尾流转。
