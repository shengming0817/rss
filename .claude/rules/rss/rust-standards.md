...
> 架构层 / Cell / Slice / contract 如何映射到 crate 见 `rust-mapping.md`。本文件只讲 Rust 语言层规范。

## 分层依赖（crate 图编译期强制）

| crate | 允许依赖 | 禁止依赖 |
|----|----------|----------|
| `rss-kernel` | std、serde、serde_yaml | runtime、adapters、cells |
| cell（组合 crate） | rss-*、contract-*、本 cell 的 slice/internal crate | 其它 Cell 的 crate |
| slice | rss-*、contract-*、（L0）兄弟 cell crate | 其它 Cell 的 crate |
| `rss-runtime` | kernel、pkg crate | cells、adapters |
| `adapter-*` | kernel、runtime | cells |
| pkg（errcode/ctx/...） | std | kernel、cells、runtime、adapters |
| assembly / bin | 所有层 | 无 |

cargo 不允许循环依赖；多余 / 未声明依赖用 `cargo-udeps`、外部 API 面用 `cargo public-api` 守。
...
级别声明为 trait 关联常量 `const CONSISTENCY: ConsistencyLevel`（类型级 marker）。L2 覆盖由 `L2-OUTBOX-ATOMICITY-COVERAGE-01` 守卫。
...
- 索引形态按阶段：pre-GA / 有序 migration 集 / 新建或空表用普通 `CREATE INDEX`（留在事务型 migration）；
  `CONCURRENTLY` 仅用于 post-GA 给已填充、有在线流量的生产表加索引。详见 `crates/adapters/postgres/migrations/README.md`。
