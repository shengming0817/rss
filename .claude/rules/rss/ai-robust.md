...
> RSS 是 GoCell 的 Rust 重写。核心方向：GoCell 因 Go 类型系统弱，大量约束靠 archtest
> （`tools/archtest/*_test.go`）+ governance rule + rustdoc 约定守；Rust 重写后**优先把约束上移到编译期**
...
- `rss validate` / `rss check` governance rule（运行期治理测试）
...
1. **类型系统 / crate 依赖图**：用 Cargo `[dependencies]`、`pub(crate)` 可见性、sealed trait、
   ...
   cell crate 依赖不到其它 cell crate、必填依赖非 `Option`、domain 类型不 derive `Serialize`
...
4. **运行期 governance 测试（`rss check` / kernel 内 `#[test]`）**：仅用于类型系统与 crate 图
...
- **trait 关联常量 marker**：语义（如 `const CONSISTENCY: ConsistencyLevel`）落到类型级，编译期固定。
