...
#[test]
fn audit_ledger_help_is_local_and_does_not_open_runtime_configuration() -> anyhow::Result<()> {
    let cases: &[(&[&str], &[&str])] = &[
        (&["audit-ledger", "--help"], &["audit-ledger", "verify"]),
        (
            &["audit-ledger", "verify", "--help"],
            &[
                "audit-ledger",
                "verify",
...
fn audit_ledger_SECRET_BAIT_assignment_fails_before_runtime() -> anyhow::Result<()> {
    let output = rss(&[
        "audit-ledger",
        "verify",
    ...
    assert!(
        stderr.contains("audit-ledger: invalid value; see --help"),
        "stderr={stderr}"
    );
    assert!(
