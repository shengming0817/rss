...
fn assert_json_coverage(json: &serde_json::Value) {
    ...
    assert_eq!(coverage["activeCount"], 7);
    assert_eq!(coverage["registeredCount"], 7);
...
fn assert_json_rows(json: &serde_json::Value) {
    ...
    assert_eq!(json["activeHttpContractCount"], 21);
    assert_eq!(json["contracts"].as_array().map(Vec::len), Some(21));
    assert_eq!(
        json["contracts"]
            .as_array()
            .into_iter()
...
fn consistency_report_cli_emits_complete_deterministic_artifacts() -> anyhow::Result<()> {
    ...
    assert!(markdown.contains("Static status: **passed** · Active HTTP contracts: **21**"));
    assert!(markdown.contains(
        "Source receipt registration (fail-closed; tests not executed): **7/7 registered** · Missing: **0**"
    ));
