...
impl Evidence {
    ...
    fn parse_with_identity(
        ...
        if evidence.run_id != run_id || evidence.run_attempt != run_attempt {
        ...
        Ok(Self {
            ...
            disk_free_bytes: evidence.disk_free_bytes,
            target_bytes: evidence.target_bytes,
            download_cache_bytes: evidence.download_cache_bytes,
            tool_cache_bytes: evidence.tool_cache_bytes,
...
fn validate_safe_integer(value: u64) -> Result<()> {
    if value > MAX_JSON_INTEGER {
        bail!("CI SLO integer exceeds the JSON safe range");
    }
    Ok(())
}
...
pub(crate) fn evaluate(
    ...
    if evidence.job_key != job {
        bail!("CI evidence job key mismatch");
    }
