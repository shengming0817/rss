...
//! INVARIANT: ASSEMBLY-LOCK-LF-CHECKOUT-01 { level = "Medium", exec = "release-check", source = "code", synthetic_red = "generated_file::tests::lf_checkout_rejects_missing_weakened_and_overridden_rules", anti_vacuity = "assembly_lock::tests::committed_locks_are_clean_and_repository_verified" } — every lock target has effective `text=set,eol=lf` before release byte comparison or generation.
//! INVARIANT: ASSEMBLY-LOCK-VERIFY-GATE-01 { level = "Medium", exec = "release-check", source = "code", synthetic_red = "assembly_lock_check_is_release_owned_once_and_ordered", anti_vacuity = "assembly_lock::tests::committed_locks_are_clean_and_repository_verified" } — the typed no-compile check occurs exactly once in release-check immediately before RuntimePlan drift validation.
