...
const TEST_ONLY_FEATURES: &[&str] = &["test-support", "l2-test-support", "internal-test-support"];
        ...
        syn::Meta::NameValue(value) if value.path.is_ident("feature") => {
            matches!(
                ...
                syn::Expr::Lit(syn::ExprLit {
                    ...
                    ..
                }) if TEST_ONLY_FEATURES.contains(&feature.value().as_str())
            )
        }
...
const EVENTING_PUBLIC_EXTERNAL_SHIPPED_DEPS: &[&str] = &["thiserror"];
    ...
    deps.iter()
        .filter(|dependency| {
            dependency.from == EVENTING_PUBLIC_CRATE
                && if dependency.is_workspace_internal {
                    !EVENTING_PUBLIC_SHIPPED_DEPS.contains(&dependency.package_name.as_str())
                } else {
                    !EVENTING_PUBLIC_EXTERNAL_SHIPPED_DEPS
        ...
        .map(|dependency| {
            finding(
                ...
                EVENTING_PUBLIC_CRATE,
                format!(
                    "{} {}.{} → `{}`：rss-eventing shipped direct dependency 只准 internal={:?} external={:?}；rss-conformance 只准 dev-dependency",
...
"#,
    ...
    fn eventing_public_shipped_allowlist_rejects_alias_build_and_target_bypasses() {
        ...
        let mut thiserror = shipped_dep("rss-eventing", "thiserror", "thiserror", "[dependencies]");
        thiserror.is_workspace_internal = false;
        assert!(check_eventing_public_dependencies(&[thiserror]).is_empty());
