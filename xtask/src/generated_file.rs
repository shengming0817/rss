...
/// Enumerate one directory through a held no-follow capability and require regular-file entries.
pub(crate) fn list_stable_regular_files(path: &Path) -> Result<Vec<OsString>> {
    list_stable_regular_files_with_hook(path, || Ok(()))
}

#[cfg(unix)]
pub(crate) fn list_stable_regular_files_with_hook(
    path: &Path,
    ...
    let capability = open_parent_directory(&path.join(".rss-directory-capability"))
        .context("generated directory is unsafe")?;
    let opened = fstat(capability.fd()).context("generated directory fstat failed")?;
    after_open()?;
    let mut entries = Vec::new();
    let mut directory =
        Dir::read_from(capability.fd()).context("generated directory open failed")?;
    for entry in &mut directory {
        let entry = entry.context("generated directory iteration failed")?;
        ...
        .context("generated entry metadata failed")?;
        ensure!(
            FileType::from_raw_mode(metadata.st_mode) == FileType::RegularFile,
            "generated directory contains a non-regular entry"
        );
        entries.push(OsStr::from_bytes(name).to_os_string());
    }
    let current = fs::symlink_metadata(path).context("generated directory was replaced")?;
    ensure!(
        current.is_dir()
            && !current.file_type().is_symlink()
            && (opened.st_dev as u64, opened.st_ino as u64) == (current.dev(), current.ino()),
        "generated directory was replaced during enumeration"
    );
    entries.sort();
    Ok(entries)
}
...
pub(crate) fn list_stable_regular_files_with_hook(
    ...
    let before = fs::symlink_metadata(path).context("generated directory metadata failed")?;
    ensure!(
    ...
    after_open()?;
    let mut entries = Vec::new();
    for entry in fs::read_dir(path).context("generated directory read failed")? {
        let entry = entry.context("generated directory iteration failed")?;
        let metadata =
            fs::symlink_metadata(entry.path()).context("generated entry metadata failed")?;
        ensure!(
    ...
    let after = fs::symlink_metadata(path).context("generated directory was replaced")?;
    ensure!(
        after.is_dir()
            && !after.file_type().is_symlink()
