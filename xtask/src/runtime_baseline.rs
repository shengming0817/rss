...
//! runtime Cargo dependencies and the shared dependency/result structs. Runtime relations are
//! enforced by their canonical typed/AST owners rather than a parallel text-anchor inventory. It intentionally keeps field-inventory drift separate from
//! `SharedRuntimeDeps` infra-only semantics, which are enforced by `runtime-deps-guard`.
//!
//! INVARIANT: RUNTIME-BASELINE-DRIFT-01 { level = "Medium", exec = "check", source = "code", synthetic_red = "tests::runtime_baseline_drift_checker_rejects_closed_mutations", anti_vacuity = "tests::runtime_baseline_drift_checker_accepts_workspace" } -- `cargo xtask runtime-baseline verify`
...
use crate::phase_helper_expand::{
    PhaseExpandError, expand_inherent_phase_method, mask_comments_and_strings,
};
...
fn runtime_plan_live_closure_findings(root: &Path) -> Result<Vec<Finding<Rule>>> {
    ...
    let required = [RUNTIME_DOMAIN_EXEC_PATH, RUNTIME_PHASE_DOMAINS_PATH];
    for path in required {
        if !root.join(path).exists() {
            return Ok(vec![finding(
                Rule::MissingInvariantOwner,
                path,
    ...
    let checks = [
        ...
        (
            exclusive_calls
                .generated_wire_domains
                .is_exact_single_call()
    ...
    findings.extend(runtime_plan_generated_domain_exclusivity_findings(root)?);
    Ok(findings)
}
...
fn listener_plan_execution_findings(root: &Path) -> Result<Vec<Finding<Rule>>> {
    ...
    let finalize_owner_inventory = finalize_owner
        .map(|method| listener_plan_execution_block_inventory(&method.block))
        .unwrap_or_default();
    let finalize_owner_tokens = finalize_owner
        .map(|method| compact_tokens(&method.block))
        .unwrap_or_default();
    ...
    let checks = [
        ...
        (
            total_inventory.projection_calls == 2
                ...
                && finalize_owner_inventory.canonical_finalizer_calls == 1
        ...
        (
            finalize_owner_inventory.finalizer_calls == 1
                && finalize_owner_inventory.canonical_finalizer_calls == 1
                && finalize_owner_tokens.contains("listener_execution_plan,")
                && finalize_owner_tokens.contains("letfinalized_listeners=finalize_listener_plan(")
                && finalize_owner_tokens.contains("let(listeners,probe_receipt,health_reporter)=finalized_listeners.into_parts();")
                && finalize_owner_tokens.contains("Ok(((listeners,probe_receipt),inventory_publisher))")
                && finalize_owner_tokens.contains("listeners,")
                && finalize_owner_tokens.contains("probe_receipt,")
                && finalize_owner_tokens.contains("inventory_publisher,"),
            RUNTIME_PHASE_FINALIZE_PATH,
            "canonical DomainsWired::finalize 必须直接消费 plan capability 后调用唯一 listener finalizer",
        ),
        (
...
fn listener_plan_execution_block_inventory(block: &syn::Block) -> ListenerPlanExecutionInventory {
    let mut inventory = ListenerPlanExecutionInventory::default();
...
const RSS_COMMAND_FAMILIES: &[(&str, Option<&str>, Option<&str>)] = &[
    ...
    ("AuditLedgerVerify", Some("is_audit_ledger_command"), None),
    ("Dlq", Some("is_dlq_command"), None),
    (
...
fn device_latent_dispatch_is_canonical(
    ...
    is_exact_path(
        ...
        && preparation_arms.len() == 2
        && preparation_arms.iter().any(|arm| {
            arm == "runtime::operator::DeviceLatentCommandPreparation::Help=>returnOk(()),"
                || arm == "runtime::operator::DeviceLatentCommandPreparation::Help=>{returnOk(());}"
        })
        && preparation_arms.iter().any(|arm| {
...
fn l2_dr_recovery_dispatch_is_canonical(
    ...
    is_exact_path(
        ...
        && preparation_arms.len() == 2
        && preparation_arms.iter().any(|arm| {
            arm == "runtime::operator::L2DrRecoveryCommandPreparation::Help=>returnOk(()),"
        })
        && preparation_arms.iter().any(|arm| {
...
fn audit_ledger_dispatch_is_canonical(
    statement: &syn::Stmt,
    command: &syn::Ident,
    args: &syn::Ident,
) -> bool {
    ...
    if compact_tokens(&branch.cond) != format!("letOperatorCommand::AuditLedgerVerify={command}")
        || branch.else_branch.is_some()
        || branch.then_branch.stmts.len() != 5
    {
        return false;
    ...
    is_exact_path(
        &prepare_call.func,
        &["runtime", "operator", "prepare_audit_ledger_verify_command"],
    ) && prepare_call.args.len() == 1
        && prepare_call
            .args
            .first()
        ...
        && preparation_arms.len() == 2
        && preparation_arms.iter().any(|arm| {
            arm == "runtime::operator::AuditLedgerVerifyCommandPreparation::Help=>returnOk(()),"
        }) && preparation_arms.iter().any(|arm| {
        arm == "runtime::operator::AuditLedgerVerifyCommandPreparation::Execute(command)=>command,"
    })
        && is_exact_path(
            &runtime_call.func,
            &["runtime", "operator", "prepare_runtime"],
        ...
        && is_exact_path(
            &run_call.func,
            &["runtime", "operator", "run_audit_ledger_verify_command"],
        ) && run_call.args.len() == 2
        && run_call
            .args
            .first()
...
fn settings_config_value_maintenance_dispatch_is_canonical(
    statement: &syn::Stmt,
    command: &syn::Ident,
    args: &syn::Ident,
) -> bool {
    ...
    if compact_tokens(&branch.cond)
        != format!("letOperatorCommand::SettingsConfigValueMaintenance={command}")
        || branch.else_branch.is_some()
        || branch.then_branch.stmts.len() != 5
    {
        return false;
    ...
    is_exact_path(
        ...
        &[
            "runtime",
            "operator",
            "prepare_settings_config_value_maintenance_command",
        ],
    ) && prepare_call.args.len() == 1
        && prepare_call
            .args
        ...
        && preparation_arms.len() == 2
        && preparation_arms.iter().any(|arm| {
            arm == "runtime::operator::SettingsConfigValueMaintenanceCommandPreparation::Help=>returnOk(()),"
                || arm == "runtime::operator::SettingsConfigValueMaintenanceCommandPreparation::Help=>{returnOk(());},"
        }) && preparation_arms.iter().any(|arm| {
        arm == "runtime::operator::SettingsConfigValueMaintenanceCommandPreparation::Execute(command)=>command,"
            || arm == "runtime::operator::SettingsConfigValueMaintenanceCommandPreparation::Execute(command,)=>command,"
    })
        && is_exact_path(
            &runtime_call.func,
...
fn dlq_dispatch_is_canonical(
    ...
    if compact_tokens(&branch.cond) != format!("letOperatorCommand::Dlq={command}")
        || branch.else_branch.is_some()
        || branch.then_branch.stmts.len() != 5
    {
        return false;
    ...
    is_exact_path(
        &prepare_call.func,
        &["runtime", "operator", "prepare_dlq_command"],
    ) && prepare_call.args.len() == 1
        && prepare_call
            .args
            .first()
        ...
        && preparation_arms.len() == 2
        && preparation_arms.iter().any(|arm| {
            arm == "runtime::operator::DlqCommandPreparation::Help=>returnOk(()),"
                || arm == "runtime::operator::DlqCommandPreparation::Help=>{returnOk(());},"
        })
        && preparation_arms.iter().any(|arm| {
            arm == "runtime::operator::DlqCommandPreparation::Execute(command)=>command,"
                || arm == "runtime::operator::DlqCommandPreparation::Execute(command,)=>command,"
        })
        && is_exact_path(
            &runtime_call.func,
        ...
        && is_exact_path(
            &run_call.func,
            &["runtime", "operator", "run_dlq_control_command"],
        )
        && run_call.args.len() == 2
        && run_call
            .args
...
fn rss_main_is_canonical(main: &syn::ItemFn) -> bool {
    ...
    let Some((args, command)) = rss_main_args_and_command(args_statement, command_statement) else {
        return false;
    };
    if !offline_dispatch_is_canonical(offline_statement, args, command) {
        return false;
    }
    let Some(operator_command) = rss_main_serving_operator_binding(serving_statement, command)
    else {
        return false;
    };
    if !migration_dispatch_is_canonical(migration_statement, operator_command, args)
        || !projection_dispatch_is_canonical(projection_statement, operator_command, args)
        || !saga_dispatch_is_canonical(saga_statement, operator_command, args)
        || !device_latent_dispatch_is_canonical(device_latent_statement, operator_command, args)
        || !l2_dr_recovery_dispatch_is_canonical(l2_dr_recovery_statement, operator_command, args)
        || !reconcile_target_dispatch_is_canonical(
            reconcile_target_statement,
            operator_command,
            args,
        )
        || !audit_ledger_dispatch_is_canonical(audit_ledger_statement, operator_command, args)
        || !settings_config_value_maintenance_dispatch_is_canonical(
            settings_statement,
            operator_command,
            args,
        )
        || !dlq_dispatch_is_canonical(dlq_statement, operator_command, args)
    {
        return false;
    }
    let Some(runtime_inputs) = rss_main_prepare_runtime_binding(prepare_statement) else {
        return false;
    };
    let Some(result) =
        rss_main_operator_match_result(result_statement, operator_command, args, runtime_inputs)
    else {
        return false;
    };
    rss_main_shutdown_and_tail_are_canonical(
        shutdown_statement,
        tail_statement,
        runtime_inputs,
        result,
...
fn rss_main_args_and_command<'a>(
    args_statement: &'a syn::Stmt,
    command_statement: &'a syn::Stmt,
) -> Option<(&'a syn::Ident, &'a syn::Ident)> {
    let syn::Stmt::Local(args_local) = args_statement else {
        return None;
    };
    let args = pat_ident(&args_local.pat)?;
    if !args_local
        .init
        .as_ref()
    ...
    let command = pat_ident(&command_local.pat)?;
    let classify_call = command_local
        .init
        .as_ref()
        .and_then(|init| direct_call_behind_runtime_context(&init.expr))?;
...
fn rss_main_serving_operator_binding<'a>(
    serving_statement: &'a syn::Stmt,
    command: &syn::Ident,
) -> Option<&'a syn::Ident> {
    ...
    let syn::Pat::Ident(operator_command) = operator_pattern.elems.first()? else {
        return None;
    };
    let serving_init = serving_local.init.as_ref()?;
    ...
    serving_is_canonical.then_some(&operator_command.ident)
}

fn rss_main_prepare_runtime_binding(prepare_statement: &syn::Stmt) -> Option<&syn::Ident> {
    let syn::Stmt::Local(prepare_local) = prepare_statement else {
        return None;
    };
    let runtime_inputs = pat_ident(&prepare_local.pat)?;
    let prepare_call = prepare_local
        .init
        .as_ref()
        .and_then(|init| direct_call_behind_runtime_context(&init.expr))?;
    if !is_exact_path(
...
fn rss_main_operator_match_result<'a>(
    result_statement: &'a syn::Stmt,
    operator_command: &syn::Ident,
    args: &syn::Ident,
    ...
    let result = pat_ident(&result_local.pat)?;
    let syn::Expr::Match(dispatch) = result_local
        .init
        .as_ref()
        .map(|init| transparent_expr(&init.expr))?
    else {
    ...
    if !is_exact_ident_path(&dispatch.expr, operator_command)
        || dispatch.arms.len() != RSS_COMMAND_FAMILIES.len()
        || !rss_operator_match_arms_are_canonical(&dispatch.arms, args, runtime_inputs)
    {
        return None;
    }
    Some(result)
...
fn rss_prepare_first_unreachable_body(variant: &str) -> Option<&'static str> {
    match variant {
        "Postgres" => Some("{unreachable!(\"postgresmigrationreturnsbeforeruntimesetup\")}"),
        "Projection" => Some("unreachable!(\"projectionusesdedicatedruntimeinputs\")"),
        "Saga" => Some("unreachable!(\"Sagapreparationreturnsbeforeruntimesetup\")"),
        "DeviceLatentInspection" => {
            Some("{unreachable!(\"DeviceLatentpreparationreturnsbeforeruntimesetup\")}")
        }
        "L2DrRecovery" => {
            Some("{unreachable!(\"L2DRrecoverypreparationreturnsbeforeruntimesetup\")}")
        }
        "ReconcileTarget" => {
            Some("{unreachable!(\"ReconcileTargetpreparationreturnsbeforeruntimesetup\")}")
        }
        "AuditLedgerVerify" => {
            Some("{unreachable!(\"AuditLedgerVerifypreparationreturnsbeforeruntimesetup\")}")
        }
        "SettingsConfigValueMaintenance" => Some(
            "{unreachable!(\"SettingsConfigValueMaintenancepreparationreturnsbeforeruntimesetup\")}",
        ),
        "Dlq" => Some("{unreachable!(\"Dlqpreparationreturnsbeforeruntimesetup\")}"),
        _ => None,
    }
}

fn rss_operator_match_arms_are_canonical(
    arms: &[syn::Arm],
    args: &syn::Ident,
    runtime_inputs: &syn::Ident,
) -> bool {
    ...
    for arm in arms {
        ...
        if let Some(expected) = rss_prepare_first_unreachable_body(&variant) {
            if compact_tokens(&arm.body) != expected {
                return false;
            }
            continue;
        }
...
fn rss_main_shutdown_and_tail_are_canonical(
    shutdown_statement: &syn::Stmt,
    tail_statement: &syn::Stmt,
    runtime_inputs: &syn::Ident,
    result: &syn::Ident,
) -> bool {
...
fn runtime_config_snapshot_live_findings(root: &Path) -> Result<Vec<Finding<Rule>>> {
    if !root.join(RUNTIME_LIB_PATH).exists() {
        return Ok(Vec::new());
    }
...
fn production_runtime_phase_config_snapshot_findings(
    ...
    if let Err(error) = ensure_expanded_phase_method(
...
fn pg_operator_signature_bindings(
    ...
    } else if name == "run_audit_ledger_verify_command" {
        "PreparedAuditLedgerVerifyCommand"
    } else if name == "run_settings_config_value_maintenance" {
        "PreparedSettingsConfigValueMaintenanceCommand"
    } else if name == "run_dlq_control_command" {
        "PreparedDlqCommand"
    } else {
        "&[String]"
    };
...
impl<'a> PgOperatorWrapperFlow<'a> {
    ...
    fn call_is_canonical(&self, call: &syn::ExprCall) -> bool {
        if self.runtime_type == "ProductionProjectionControlRuntime"
            || self.runtime_type == "ProductionAuditLedgerVerifyRuntime"
            || self.runtime_type == "ProductionDlqControlRuntime"
        {
...
fn pg_operator_module_graph_is_exact(files: &BTreeMap<String, syn::File>) -> bool {
    ...
    let audit_ledger_prepare = files
        .get(RUNTIME_OPERATOR_AUDIT_PATH)
        .is_some_and(audit_ledger_prepare_command_is_exact);
    let dlq_prepare = files
        .get(RUNTIME_OPERATOR_DLQ_PATH)
        .is_some_and(dlq_prepare_command_is_exact);
    let settings_prepare = files
        .get(RUNTIME_OPERATOR_SETTINGS_PATH)
        .is_some_and(settings_prepare_command_is_exact);
    let device_latent_prepare = files
        .get(RUNTIME_OPERATOR_DEVICE_LATENT_PATH)
        .is_some_and(device_latent_prepare_command_is_exact);
    let l2_dr_recovery_prepare = files
        .get(RUNTIME_OPERATOR_DR_RECOVERY_PATH)
        .is_some_and(l2_dr_recovery_prepare_command_is_exact);
    common
        && saga
        && projection_prepare
        && audit_ledger_prepare
        && dlq_prepare
        && settings_prepare
        && device_latent_prepare
        && l2_dr_recovery_prepare
}
...
/// Medium nail: `prepare_audit_ledger_verify_command` may only funnel through clap stdin prep.
fn audit_ledger_prepare_command_is_exact(file: &syn::File) -> bool {
    let prepared = file
        .items
        .iter()
        .filter_map(|item| match item {
            syn::Item::Struct(item) if item.ident == "PreparedAuditLedgerVerifyCommand" => {
                Some(item)
            }
            _ => None,
        })
    ...
    let preparations = file
        ...
        .iter()
        .filter_map(|item| match item {
            syn::Item::Enum(item) if item.ident == "AuditLedgerVerifyCommandPreparation" => {
                Some(item)
            }
            _ => None,
        })
        .collect::<Vec<_>>();
    let prepare_functions = production_functions_named(file, "prepare_audit_ledger_verify_command");
    let carriers_are_closed = matches!(prepared.as_slice(), [item]
        if matches!(&item.vis, syn::Visibility::Public(_))
            && matches!(&item.fields, syn::Fields::Unnamed(fields)
                if fields.unnamed.len() == 1
                    && matches!(fields.unnamed[0].vis, syn::Visibility::Inherited)
                    && compact_tokens(&fields.unnamed[0].ty) == "AuditLedgerVerifyArgs"))
        && matches!(preparations.as_slice(), [item]
            if matches!(&item.vis, syn::Visibility::Public(_))
                && item.variants.len() == 2
                && item.variants.iter().any(|variant| variant.ident == "Help"
                ...
                && item.variants.iter().any(|variant| variant.ident == "Execute"
                    && matches!(&variant.fields, syn::Fields::Unnamed(fields)
                        if fields.unnamed.len() == 1
                            && compact_tokens(&fields.unnamed[0].ty)
                                == "PreparedAuditLedgerVerifyCommand")));
    let prepare_is_exact = matches!(prepare_functions.as_slice(), [prepare]
        if exact_named_typed_input(&prepare.sig, 0, "args", "&[String]")
            && compact_tokens(&prepare.sig.output)
                == "->anyhow::Result<AuditLedgerVerifyCommandPreparation>"
            && exact_named_path_call_count(&prepare.block, &["std", "io", "stdin"]) == 1
            && exact_path_call_argument_count(
                &prepare.block,
                &["clap_cli", "prepare_audit_ledger_verify_command_with_stdin"],
                0,
                "args",
            ) == 1
            && method_call_count_in_block(&prepare.block, "lock") == 1);
...
/// Medium nail: `prepare_settings_config_value_maintenance_command` may only funnel through clap
/// stdin prep.
fn dlq_prepare_command_is_exact(file: &syn::File) -> bool {
    let prepared = file
        .items
        .iter()
        .filter_map(|item| match item {
            syn::Item::Struct(item) if item.ident == "PreparedDlqCommand" => Some(item),
            _ => None,
        })
        .collect::<Vec<_>>();
    let preparations = file
        ...
        .iter()
        .filter_map(|item| match item {
            syn::Item::Enum(item) if item.ident == "DlqCommandPreparation" => Some(item),
            _ => None,
        })
        .collect::<Vec<_>>();
    let prepare_functions = production_functions_named(file, "prepare_dlq_command");
    let carriers_are_closed = matches!(prepared.as_slice(), [item]
        if matches!(&item.vis, syn::Visibility::Public(_))
            && matches!(&item.fields, syn::Fields::Unnamed(fields)
                if fields.unnamed.len() == 1
                    && matches!(fields.unnamed[0].vis, syn::Visibility::Inherited)
                    && compact_tokens(&fields.unnamed[0].ty) == "DlqCliArgs"))
        && matches!(preparations.as_slice(), [item]
            if matches!(&item.vis, syn::Visibility::Public(_))
                && item.variants.len() == 2
                && item.variants.iter().any(|variant| variant.ident == "Help"
    ...
    let prepare_is_exact = matches!(prepare_functions.as_slice(), [prepare]
        if exact_named_typed_input(&prepare.sig, 0, "args", "&[String]")
            ...
            && exact_path_call_argument_count(
                &prepare.block,
                &["clap_cli", "prepare_dlq_command_with_stdin"],
                0,
                "args",
            ) == 1
            && method_call_count_in_block(&prepare.block, "lock") == 1);
...
/// Medium nail: `prepare_settings_config_value_maintenance_command` may only funnel through clap
/// stdin prep.
fn settings_prepare_command_is_exact(file: &syn::File) -> bool {
    let prepared = file
        .items
        .iter()
        .filter_map(|item| match item {
            syn::Item::Struct(item)
                if item.ident == "PreparedSettingsConfigValueMaintenanceCommand" =>
            {
                Some(item)
            }
            _ => None,
    ...
    let preparations = file
        ...
        .filter_map(|item| match item {
            syn::Item::Enum(item)
                if item.ident == "SettingsConfigValueMaintenanceCommandPreparation" =>
            {
                Some(item)
            }
            _ => None,
    ...
    let prepare_functions =
        production_functions_named(file, "prepare_settings_config_value_maintenance_command");
    let carriers_are_closed = matches!(prepared.as_slice(), [item]
        if matches!(&item.vis, syn::Visibility::Public(_))
            && matches!(&item.fields, syn::Fields::Unnamed(fields)
                if fields.unnamed.len() == 1
                    && matches!(fields.unnamed[0].vis, syn::Visibility::Inherited)
                    && compact_tokens(&fields.unnamed[0].ty)
                        == "SettingsConfigValueMaintenanceArgs"))
        && matches!(preparations.as_slice(), [item]
            if matches!(&item.vis, syn::Visibility::Public(_))
                && item.variants.len() == 2
                && item.variants.iter().any(|variant| variant.ident == "Help"
    ...
    let prepare_is_exact = matches!(prepare_functions.as_slice(), [prepare]
        if exact_named_typed_input(&prepare.sig, 0, "args", "&[String]")
            ...
            && exact_path_call_argument_count(
                ...
                &[
                    "clap_cli",
                    "prepare_settings_config_value_maintenance_command_with_stdin",
                ],
                0,
                "args",
            ) == 1
...
/// Medium nail: `prepare_l2_dr_recovery_command` may only funnel through clap stdin prep.
fn l2_dr_recovery_prepare_command_is_exact(file: &syn::File) -> bool {
    let prepared = file
        .items
        .iter()
        .filter_map(|item| match item {
            syn::Item::Struct(item) if item.ident == "PreparedL2DrRecoveryCommand" => Some(item),
            _ => None,
        })
        .collect::<Vec<_>>();
    let preparations = file
        ...
        .iter()
        .filter_map(|item| match item {
            syn::Item::Enum(item) if item.ident == "L2DrRecoveryCommandPreparation" => Some(item),
            _ => None,
        })
        .collect::<Vec<_>>();
    let prepare_functions = production_functions_named(file, "prepare_l2_dr_recovery_command");
    let carriers_are_closed = matches!(prepared.as_slice(), [item]
        if matches!(&item.vis, syn::Visibility::Public(_))
            && matches!(&item.fields, syn::Fields::Unnamed(fields)
                if fields.unnamed.len() == 1
                    && matches!(fields.unnamed[0].vis, syn::Visibility::Inherited)
                    && compact_tokens(&fields.unnamed[0].ty) == "L2DrRecoveryCliArgs"))
        && matches!(preparations.as_slice(), [item]
            if matches!(&item.vis, syn::Visibility::Public(_))
                && item.variants.len() == 2
                && item.variants.iter().any(|variant| variant.ident == "Help"
    ...
    let prepare_is_exact = matches!(prepare_functions.as_slice(), [prepare]
        if exact_named_typed_input(&prepare.sig, 0, "args", "&[String]")
            ...
            && exact_path_call_argument_count(
                &prepare.block,
                &["clap_cli", "prepare_l2_dr_recovery_command_with_stdin"],
                0,
                "args",
            ) == 1
            && method_call_count_in_block(&prepare.block, "lock") == 1);
...
/// Medium nail: `prepare_device_latent_command` may only funnel through clap stdin prep.
fn device_latent_prepare_command_is_exact(file: &syn::File) -> bool {
    let prepared = file
        .items
        .iter()
        .filter_map(|item| match item {
            syn::Item::Struct(item) if item.ident == "PreparedDeviceLatentCommand" => Some(item),
            _ => None,
        })
        .collect::<Vec<_>>();
    let preparations = file
        ...
        .iter()
        .filter_map(|item| match item {
            syn::Item::Enum(item) if item.ident == "DeviceLatentCommandPreparation" => Some(item),
            _ => None,
        })
        .collect::<Vec<_>>();
    let prepare_functions = production_functions_named(file, "prepare_device_latent_command");
    let carriers_are_closed = matches!(prepared.as_slice(), [item]
        if matches!(&item.vis, syn::Visibility::Public(_))
            && matches!(&item.fields, syn::Fields::Unnamed(fields)
                if fields.unnamed.len() == 1
                    && matches!(fields.unnamed[0].vis, syn::Visibility::Inherited)
                    && compact_tokens(&fields.unnamed[0].ty) == "DeviceLatentInspectionCommand"))
        && matches!(preparations.as_slice(), [item]
            if matches!(&item.vis, syn::Visibility::Public(_))
                && item.variants.len() == 2
                && item.variants.iter().any(|variant| variant.ident == "Help"
    ...
    let prepare_is_exact = matches!(prepare_functions.as_slice(), [prepare]
        if exact_named_typed_input(&prepare.sig, 0, "args", "&[String]")
            ...
            && exact_path_call_argument_count(
                &prepare.block,
                &["clap_cli", "prepare_device_latent_command_with_stdin"],
                0,
                "args",
            ) == 1
            && method_call_count_in_block(&prepare.block, "lock") == 1);
...
fn runtime_secret_transfer_live_findings(root: &Path) -> Result<Vec<Finding<Rule>>> {
    ...
    let (inventory, env_secret_structs) = collect_runtime_secret_flow_inventory(root)?;
    let mut findings = Vec::new();
...
fn collect_runtime_secret_flow_inventory(root: &Path) -> Result<(SecretFlowInventory, usize)> {
    let mut paths = Vec::new();
    collect_rust_sources(&root.join(RUNTIME_SRC_PATH), &mut paths)?;
    let production_sources = production_module_sources(&paths)?;
...
fn runtime_launch_kernel_owner_findings(root: &Path) -> Result<Vec<Finding<Rule>>> {
    ...
    let launch = unique_production_async_function(&kernel, "launch");
    let launch_until = unique_production_async_function(&kernel, "launch_until");
    let execute_launch = unique_production_async_function(&kernel, "execute_launch");
    let finish_launch = unique_production_async_function(&kernel, "finish_launch");
    let install_signal = unique_production_function(&kernel, "install_shutdown_signal");
    let spawn_drain = unique_production_function(&kernel, "spawn_drain");
    let register_outputs = unique_production_function(&kernel, "register_lifecycle_outputs");
    let register_module = unique_production_function(&kernel, "register_module_output");
    let shutdown_new = unique_production_inherent_method(&kernel, "ShutdownOwner", "new");
    let stage_resource =
        unique_production_inherent_method(&kernel, "LaunchTransaction", "stage_resource");
    let transaction_commit =
        unique_production_inherent_method(&kernel, "LaunchTransaction", "commit");
    let register_listener = unique_production_inherent_method(
        &kernel,
        "LaunchRegistrar",
        "register_listener_with_token",
    );
    let complete_activation =
        unique_production_inherent_method(&kernel, "LaunchRegistrar", "complete");
    let tokens = |function: Option<&syn::ItemFn>| {
        function
            .map(|function| compact_tokens(&function.block))
            .unwrap_or_default()
    };
    let method_tokens = |method: Option<&syn::ImplItemFn>| {
        method
            .map(|method| compact_tokens(&method.block))
            .unwrap_or_default()
    };
    let execute_tokens = tokens(execute_launch);
    let finish_tokens = tokens(finish_launch);
    let install_signal_tokens = tokens(install_signal);
    let spawn_drain_tokens = tokens(spawn_drain);
    let register_outputs_tokens = tokens(register_outputs);
    let register_module_tokens = tokens(register_module);
    let shutdown_new_tokens = method_tokens(shutdown_new);
    let stage_resource_tokens = method_tokens(stage_resource);
    let transaction_commit_tokens = method_tokens(transaction_commit);
    let register_listener_tokens = method_tokens(register_listener);
    let complete_activation_tokens = method_tokens(complete_activation);
    let preserves_primary_launch_error = preserve_launch_error_is_canonical(&kernel);
    let lifecycle_checks = [
        (
            "launch-call-graph",
            launch.is_some_and(|function| {
                exact_named_path_call_count(&function.block, &["launch_until"]) == 1
                    && compact_tokens(&function.block)
                        .contains("launch_until(plan,install_shutdown_signal).await")
            }) && launch_until.is_some_and(|function| {
                exact_named_path_call_count(&function.block, &["execute_launch"]) == 1
                    && exact_named_path_call_count(&function.block, &["finish_launch"]) == 1
            }),
        ),
        (
            "stack-owner",
            shutdown_new_tokens.contains("ShutdownStack::new(CancellationToken::new())")
                && production_exact_path_call_count_in_file(&kernel, &["ShutdownStack", "new"])
                    == 1,
        ),
        (
            "batch-transfer-call",
        ...
        (
            ...
            register_outputs.is_some_and(|function| {
                exact_named_path_call_count(&function.block, &["register_module_output"]) == 2
            }) && production_exact_path_call_count_in_file(
                &kernel,
        ...
        (
            "signal-call",
            install_signal.is_some_and(|function| {
                exact_named_path_call_count(&function.block, &["wait_for_shutdown_signal"]) == 1
            }) && install_signal_tokens.contains("letshutdown=wait_for_shutdown_signal()?")
                && production_exact_path_call_count_in_file(
                    &kernel,
        ...
        (
            "transaction-mint",
            execute_tokens.contains("letmuttransaction=LaunchTransaction{stack}"),
        ),
        (
            "receipt-prepare",
            execute_tokens.contains("adapter.prepare(probe_receipt,&muttransaction).await?"),
        ),
        (
            "registrar-activation",
            execute_tokens.contains("Adapter::activate(prepared,transaction.commit())?"),
        ),
        (
            "activated-ready",
            execute_tokens.contains("letreadiness=on_ready(activated.into_inventory())"),
        ),
        (
            "ready-signal-race",
            execute_tokens.contains("result=&mutshutdown=>returnresult"),
        ),
        (
            "drain",
            spawn_drain_tokens
                .contains("stack.shutdown_within(total_drain_budget.duration()).await")
                && finish_tokens.contains("letdrain=spawn_drain(&runtime,stack,total_drain_budget)"),
        ),
        (
            "staged-resource",
            stage_resource_tokens.contains("self.stack.register_detached(resource)"),
        ),
        (
            "empty-registrar",
            transaction_commit_tokens.contains("listener_count:0"),
        ),
        (
            "registered-listener",
            register_listener_tokens.contains("self.stack.register_with_token(make)"),
        ),
        (
            "closed-worker-admission-policy",
            register_module_tokens.contains(
        ...
        (
            ...
            shutdown_new_tokens.contains("CancellationToken::new()")
                && production_exact_path_call_count_in_file(
                    &kernel,
        ...
        (
            "listener-count",
            register_listener_tokens.contains("self.listener_count+=1"),
        ),
        (
            "nonempty-activation",
            complete_activation_tokens.contains("self.listener_count>0"),
        ),
        (
            "activated-capability",
            complete_activation_tokens.contains("Ok(Activated{inventory})"),
        ),
        (
            "typed-batch-destructure",
            register_outputs_tokens
                .contains("letLaunchLifecycleBatches{provider,domain}=lifecycle_batches"),
        ),
        (
            "provider-role",
            register_outputs_tokens.contains("register_module_output(stack,provider.0)"),
        ),
        (
            "domain-role",
            register_outputs_tokens.contains("register_module_output(stack,domain.0)"),
        ),
        (
            "batch-error-order",
            register_outputs_tokens.contains("provider_result?;domain_result"),
        ),
        (
            "dual-error-arm",
            unique_production_function(&kernel, "preserve_launch_error")
                .is_some_and(|function| {
                    compact_tokens(&function.block)
                        .contains("(Err(launch_error),Err(drain_error))")
                }),
        ),
...
fn runtime_plan_generated_domain_exclusivity_findings(root: &Path) -> Result<Vec<Finding<Rule>>> {
    ...
    for source_path in runtime_sources {
        ...
        let file = syn::parse_file(&source).with_context(|| {
            format!("解析 runtime production source {} 失败", relative.display())
        })?;
        if let Some(factory) =
...
fn visit_expanded_phase_method(
    ...
    let expanded = expand_inherent_phase_method(source, file, owner, entry)?;
    let block = syn::parse_str::<syn::Block>(&format!("{{{}}}", expanded.virtual_source)).map_err(
        |error| {
            PhaseExpandError::Parse(format!(
                "expanded {owner}::{entry} body is invalid: {error}"
            ))
        },
    )?;
    visitor.visit_block(&block);
...
fn ensure_expanded_phase_method(
    source: &str,
    file: &syn::File,
    owner: &str,
    entry: &str,
    ...
    let expanded = expand_inherent_phase_method(source, file, owner, entry)?;
    if expanded.virtual_source.trim().is_empty() {
        Err(PhaseExpandError::Parse(format!(
            "expanded {owner}::{entry} body is empty"
        )))
    } else {
        Ok(())
    }
...
mod tests {
    ...
    fn copy_runtime_evidence_sources(root: &Path) -> Result<()> {
    ...
    fn runtime_evidence_fixture(name: &str) -> Result<PathBuf> {
        let root = unique_tmp(name);
        copy_runtime_evidence_sources(&root)?;
    ...
    fn postgres_setup_transaction_rejects_missing_live_edges() -> Result<()> {
        ...
        let source = fs::read_to_string(&target)?;
        let production = source.replacen(
            "    #[cfg(any(test, feature = \"test-support\", feature = \"fault-matrix-test-support\"))]\n",
            "",
            1,
        );
    ...
    -> Result<()> {
        let root = runtime_evidence_fixture("service-token-replay-missing-call")?;
        assert!(runtime_evidence_is_canonical(&root)?);
        mutate_replay_file(
            &root,
            RUNTIME_OPERATOR_PROJECTION_PATH,
            "build_projection_operator_token_provider(",
...
"#,
        ...
        let root = runtime_evidence_fixture("service-token-replay-dead-helper")?;
        ...
        let root = runtime_evidence_fixture("service-token-replay-test-bait")?;
        let operator = root.join(RUNTIME_OPERATOR_PATH);
        let source = fs::read_to_string(&operator)?;
        write(
            &operator,
        ...
        let root = runtime_evidence_fixture("service-token-replay-process-local")?;
        let operator = root.join(RUNTIME_OPERATOR_PATH);
        let source = fs::read_to_string(&operator)?;
        write(
            &operator,
        ...
        let root = runtime_evidence_fixture("service-token-replay-widened-owner")?;
        mutate_replay_file(
            &root,
            RUNTIME_OIDC_PATH,
...
"#;
    ...
    // reason: one closed projection mutation matrix must exercise every carrier generation.
...
"#,
        ...
        for (name, mutation, expected_subject, expected_predicate) in cases {
            let root = runtime_evidence_fixture(name)?;
            write(
    ...
    #[test]
    fn runtime_launch_kernel_owner_rejects_dead_helper_relocation() -> Result<()> {
        let workspace = workspace_root()?;
        let source = fs::read_to_string(workspace.join(RUNTIMEEXEC_PATH))?;
        let cases = [
            (
                "signal",
                "let shutdown = wait_for_shutdown_signal()?;",
                "let shutdown = relocated_shutdown_signal()?;",
                r#"
fn relocated_shutdown_signal() -> anyhow::Result<impl Future<Output = anyhow::Result<()>>> {
    wait_for_shutdown_signal()
}
"#,
            ),
            (
                "readiness",
                "let readiness = on_ready(activated.into_inventory());",
                "let readiness = relocated_readiness(on_ready, activated);",
                r#"
fn relocated_readiness<ReadyHook, Inventory, Ready>(
    on_ready: ReadyHook,
    activated: Activated<Inventory>,
) -> Ready
where
    ReadyHook: FnOnce(Inventory) -> Ready,
{
    let readiness = on_ready(activated.into_inventory());
    readiness
}
"#,
            ...
            (
                "drain",
                "report_shutdown_failures(stack.shutdown_within(total_drain_budget.duration()).await)",
                "relocated_drain(stack, total_drain_budget).await",
                r#"
async fn relocated_drain(
    stack: ShutdownStack,
...
"#,
        ...
        for (case, before, replacement, dead_helper) in cases {
            let root =
                runtime_evidence_fixture(&format!("runtime-launch-dead-helper-relocation-{case}"))?;
            let mutated = source.replacen(before, replacement, 1) + dead_helper;
            anyhow::ensure!(mutated != source, "{case} relocation must be live");
            write(&root.join(RUNTIMEEXEC_PATH), &mutated)?;
            let findings = runtime_launch_kernel_owner_findings(&root)?;
            assert!(
                findings.iter().any(|finding| {
                ...
                "{case} evidence relocated outside the canonical launch graph must fail: {findings:?}"
            );
        }
        Ok(())
    }
...
"#;
    ...
    #[test]
    fn runtime_config_snapshot_live_accepts_workspace() -> Result<()> {
        let root = workspace_root()?;
        assert_eq!(
            runtime_config_snapshot_live_findings(&root)?,
            Vec::<Finding<Rule>>::new(),
            "the top-level production config snapshot checker is the anti-vacuity green"
        );
        Ok(())
    }
...
"#;
        ...
        for (label, mutated) in [
            ...
            (
                ...
                canonical_rss.replace(
                    ...
                    "runtime::operator::DeviceLatentCommandPreparation::Help => { let _forbidden = runtime::operator::prepare_device_latent_runtime()?; return Ok(()); }",
                ),
            ),
            (
            ...
            (
                "rss L2DrRecovery prepares runtime before argv and stdin",
                canonical_rss.replace(
            ...
            (
                "rss L2DrRecovery help constructs runtime",
                canonical_rss.replace(
                    "runtime::operator::L2DrRecoveryCommandPreparation::Help => return Ok(()),",
                    "runtime::operator::L2DrRecoveryCommandPreparation::Help => { let _forbidden = runtime::operator::prepare_runtime()?; return Ok(()); }",
                ),
            ),
            (
                "rss Dlq prepares runtime before argv and stdin",
                canonical_rss.replace(
            ...
            (
                "rss Dlq transfers the wrong prepared command",
                canonical_rss.replace(
                    "runtime::operator::run_dlq_control_command(prepared, &runtime_inputs)",
                    "runtime::operator::run_dlq_control_command(other_prepared, &runtime_inputs)",
                ),
            ),
            (
                "rss Dlq help constructs runtime",
                canonical_rss.replace(
                    "runtime::operator::DlqCommandPreparation::Help => return Ok(()),",
                    "runtime::operator::DlqCommandPreparation::Help => { let _forbidden = runtime::operator::prepare_runtime()?; return Ok(()); }",
                ),
            ),
            (
                "rss AuditLedger prepares runtime before argv and stdin",
                canonical_rss.replace(
            ...
            (
                "rss AuditLedger transfers the wrong prepared command",
                canonical_rss.replace(
                    "runtime::operator::run_audit_ledger_verify_command(prepared, &runtime_inputs)",
                    "runtime::operator::run_audit_ledger_verify_command(other_prepared, &runtime_inputs)",
                ),
            ),
            (
                "rss AuditLedger help constructs runtime",
                canonical_rss.replace(
                    "runtime::operator::AuditLedgerVerifyCommandPreparation::Help => return Ok(()),",
                    "runtime::operator::AuditLedgerVerifyCommandPreparation::Help => { let _forbidden = runtime::operator::prepare_runtime()?; return Ok(()); }",
                ),
            ),
            (
                "rss Settings prepares runtime before argv and stdin",
                canonical_rss.replace(
            ...
            (
                "rss Settings transfers the wrong prepared command",
                canonical_rss.replace(
                    "runtime::operator::run_settings_config_value_maintenance(prepared, &runtime_inputs)",
                    "runtime::operator::run_settings_config_value_maintenance(other_prepared, &runtime_inputs)",
                ),
            ),
            (
                "rss Settings help constructs runtime",
                canonical_rss.replace(
                    "runtime::operator::SettingsConfigValueMaintenanceCommandPreparation::Help => return Ok(()),",
                    "runtime::operator::SettingsConfigValueMaintenanceCommandPreparation::Help => { let _forbidden = runtime::operator::prepare_runtime()?; return Ok(()); }",
                ),
            ),
        ] {
    ...
    fn runtime_pg_redis_snapshot_wiring() -> Result<()> {
        ...
        for operator_call in [
            ...
            "run_audit_ledger_verify_command(prepared, &runtime_inputs)",
            "run_dlq_control_command(prepared, &runtime_inputs)",
            ...
            "run_settings_config_value_maintenance(prepared, &runtime_inputs)",
    ...
    #[test]
    fn runtime_audit_ledger_operator_preparation_rejects_bypasses() -> Result<()> {
        let source = fs::read_to_string(workspace_root()?.join(RUNTIME_OPERATOR_AUDIT_PATH))?;
        let file = syn::parse_file(&source)?;
        assert!(
            audit_ledger_prepare_command_is_exact(&file),
            "audit-ledger prepare clap funnel is the anti-vacuity green"
        );
        for (label, mutated) in [
            (
                "prepared command exposes its token-bearing field",
                source.replace(
                    ...
                    "pub struct PreparedAuditLedgerVerifyCommand(pub AuditLedgerVerifyArgs);",
                ),
            ),
            (
                "argv preparation is renamed out of the binary funnel",
                source.replace(
                    ...
                    "pub fn prepare_audit_ledger_verify_command_legacy(\n    args: &[String],\n)",
                ),
            ),
            (
                "prepare body bypasses clap stdin funnel",
                source.replace(
                    ...
                    "clap_cli::prepare_audit_ledger_verify_command_with_stdin_legacy(args, &mut stdin.lock())",
                ),
            ),
        ] {
            assert_ne!(mutated, source, "{label} mutation must be live");
            assert!(
                !audit_ledger_prepare_command_is_exact(&syn::parse_file(&mutated)?),
                "audit-ledger prepare gate must reject {label}"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn runtime_dlq_operator_preparation_rejects_bypasses() -> Result<()> {
        let source = fs::read_to_string(workspace_root()?.join(RUNTIME_OPERATOR_DLQ_PATH))?;
        let file = syn::parse_file(&source)?;
        assert!(
            dlq_prepare_command_is_exact(&file),
            "dlq prepare clap funnel is the anti-vacuity green"
        );
        for (label, mutated) in [
            (
                "prepared command exposes its token-bearing field",
                source.replace(
                    ...
                    "pub struct PreparedDlqCommand(pub DlqCliArgs);",
                ),
            ),
            (
                "argv preparation is renamed out of the binary funnel",
                source.replace(
                    "pub fn prepare_dlq_command(args: &[String])",
                    "pub fn prepare_dlq_command_legacy(args: &[String])",
                ),
            ),
            (
                "prepare body bypasses clap stdin funnel",
                source.replace(
                    ...
                    "clap_cli::prepare_dlq_command_with_stdin_legacy(args, &mut stdin.lock())",
                ),
            ),
        ] {
            assert_ne!(mutated, source, "{label} mutation must be live");
            assert!(
                !dlq_prepare_command_is_exact(&syn::parse_file(&mutated)?),
                "dlq prepare gate must reject {label}"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn runtime_settings_operator_preparation_rejects_bypasses() -> Result<()> {
        let source = fs::read_to_string(workspace_root()?.join(RUNTIME_OPERATOR_SETTINGS_PATH))?;
        let file = syn::parse_file(&source)?;
        assert!(
            settings_prepare_command_is_exact(&file),
            "settings prepare clap funnel is the anti-vacuity green"
        );
        for (label, mutated) in [
            (
                "prepared command exposes its token-bearing field",
                source.replace(
                    ...
                    "pub struct PreparedSettingsConfigValueMaintenanceCommand(pub SettingsConfigValueMaintenanceArgs);",
                ),
            ),
            (
                "argv preparation is renamed out of the binary funnel",
                source.replace(
                    ...
                    "pub fn prepare_settings_config_value_maintenance_command_legacy(\n    args: &[String],\n)",
                ),
            ),
            (
                "prepare body bypasses clap stdin funnel",
                source.replace(
                    ...
                    "clap_cli::prepare_settings_config_value_maintenance_command_with_stdin_legacy(args, &mut stdin.lock())",
                ),
            ),
        ] {
            assert_ne!(mutated, source, "{label} mutation must be live");
            assert!(
                !settings_prepare_command_is_exact(&syn::parse_file(&mutated)?),
                "settings prepare gate must reject {label}"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn runtime_device_latent_operator_preparation_rejects_bypasses() -> Result<()> {
        let source =
            fs::read_to_string(workspace_root()?.join(RUNTIME_OPERATOR_DEVICE_LATENT_PATH))?;
        let file = syn::parse_file(&source)?;
        assert!(
            device_latent_prepare_command_is_exact(&file),
            "device-latent prepare clap funnel is the anti-vacuity green"
        );
        for (label, mutated) in [
            (
                "prepared command exposes its token-bearing field",
                source.replace(
                    ...
                    "pub struct PreparedDeviceLatentCommand(pub DeviceLatentInspectionCommand);",
                ),
            ),
            (
                "argv preparation is renamed out of the binary funnel",
                source.replace(
                    ...
                    "pub fn prepare_device_latent_command_legacy(\n    args: &[String],\n)",
                ),
            ),
            (
                "prepare body bypasses clap stdin funnel",
                source.replace(
                    ...
                    "clap_cli::prepare_device_latent_command_with_stdin_legacy(args, &mut stdin.lock())",
                ),
            ),
        ] {
            assert_ne!(mutated, source, "{label} mutation must be live");
            assert!(
                !device_latent_prepare_command_is_exact(&syn::parse_file(&mutated)?),
                "device-latent prepare gate must reject {label}"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn runtime_l2_dr_recovery_operator_preparation_rejects_bypasses() -> Result<()> {
        let source = fs::read_to_string(workspace_root()?.join(RUNTIME_OPERATOR_DR_RECOVERY_PATH))?;
        let file = syn::parse_file(&source)?;
        assert!(
            l2_dr_recovery_prepare_command_is_exact(&file),
            "l2-dr-recovery prepare clap funnel is the anti-vacuity green"
        );
        for (label, mutated) in [
            (
                "help variant regains a payload",
                source.replace(
            ...
            (
                ...
                source.replace(
                    ...
                    "pub fn prepare_l2_dr_recovery_command_legacy(\n    args: &[String],\n)",
                ),
            ),
            (
                "prepare body bypasses clap stdin funnel",
                source.replace(
                    ...
                    "clap_cli::prepare_l2_dr_recovery_command_with_stdin_legacy(args, &mut stdin.lock())",
                ),
            ),
        ] {
            assert_ne!(mutated, source, "{label} mutation must be live");
            assert!(
                !l2_dr_recovery_prepare_command_is_exact(&syn::parse_file(&mutated)?),
                "l2-dr-recovery prepare gate must reject {label}"
            );
        }
        Ok(())
    }
...
"#;
    ...
    #[test]
    fn runtime_plan_live_closure_reports_missing_owner_with_v2_rule() -> Result<()> {
        let root = runtime_plan_live_closure_fixture("runtime-plan-live-closure-missing-owner")?;
        fs::remove_file(root.join(RUNTIME_DOMAIN_EXEC_PATH))?;
        let findings = runtime_plan_live_closure_findings(&root)?;
        assert_eq!(findings.len(), 1, "{findings:?}");
        assert_eq!(findings[0].rule, Rule::MissingInvariantOwner);
        assert_eq!(findings[0].subject, RUNTIME_DOMAIN_EXEC_PATH);
        Ok(())
    }
    ...
    #[test]
    fn runtime_placement_plan_execution_rejects_missing_structural_evidence() -> Result<()> {
    ...
    #[test]
    fn runtime_listener_plan_execution_rejects_dead_finalizer_relocation() -> Result<()> {
        let root = listener_plan_fixture("runtime-listener-plan-dead-finalizer-relocation")?;
        let path = root.join(RUNTIME_PHASE_FINALIZE_PATH);
        let source = fs::read_to_string(&path)?;
        let mut mutated = source.replacen(
            "let finalized_listeners = finalize_listener_plan(FinalizeListenerPlanInputs {",
            "let finalized_listeners = bypass_listener_finalization(FinalizeListenerPlanInputs {",
            1,
        );
        anyhow::ensure!(mutated != source, "finalizer relocation must be live");
        mutated.push_str(
            r#"
fn dead_finalizer_evidence() {
    let finalized_listeners = finalize_listener_plan(FinalizeListenerPlanInputs {
        execution_plan: listener_execution_plan,
        config: context.config(),
        ...
        rate_limiter,
        metrics: metrics_exporter,
        framework_routes: crate::runtime_inventory::RuntimeInventoryRoutes::new(inventory_reader),
    });
    drop(finalized_listeners);
}
"#,
        );
        write(&path, &mutated)?;
        ...
        assert!(
            findings.iter().any(|finding| {
                finding.subject == RUNTIME_PHASE_FINALIZE_PATH
                    && finding.detail.contains("canonical DomainsWired::finalize")
            }),
            "dead helper finalizer evidence must not satisfy the canonical phase owner: {findings:?}"
        );
        Ok(())
    }
    ...
    fn runtime_baseline_drift_checker_accepts_workspace() -> Result<()> {
        ...
        let report = collect_report(&root)?;
        assert_eq!(
            baseline_drift_findings(&root, &report.rendered)?,
            Vec::<Finding<Rule>>::new()
        );
        Ok(())
    }
    ...
    fn runtime_baseline_drift_checker_rejects_closed_mutations() -> Result<()> {
        for mutation in [BaselineMutation::Missing, BaselineMutation::Drift] {
            let root = unique_tmp(&format!("runtime-baseline-{mutation:?}"));
            if matches!(mutation, BaselineMutation::Drift) {
                write(&root.join(BASELINE_PATH), "stale baseline\n")?;
            }
            let findings = baseline_drift_findings(&root, "canonical baseline\n")?;
            assert_eq!(findings.len(), 1, "{mutation:?}");
            assert_eq!(
                findings[0].rule,
                match mutation {
                    BaselineMutation::Missing => Rule::MissingBaseline,
                    BaselineMutation::Drift => Rule::Drift,
                }
            );
            assert_eq!(findings[0].subject, BASELINE_PATH);
        }
        Ok(())
    }

    fn copy_workspace_sources(root: &Path, relative_paths: &[&str]) -> Result<()> {
        let workspace = workspace_root()?;
    ...
    #[derive(Debug, Clone, Copy)]
    enum BinaryLifecycleMutation {
        ServerDiscardedPrepare,
        ProjectionGenericPrepare,
        OperatorDuplicatePrepare,
    }

    #[test]
    fn runtime_binary_snapshot_lifecycle_checker_accepts_workspace() -> Result<()> {
        assert_eq!(
            runtime_binary_config_findings(&workspace_root()?)?,
            Vec::<Finding<Rule>>::new()
        );
        Ok(())
    }
    ...
    #[test]
    fn runtime_binary_snapshot_lifecycle_checker_rejects_closed_mutations() -> Result<()> {
        for mutation in [
            BinaryLifecycleMutation::ServerDiscardedPrepare,
            BinaryLifecycleMutation::ProjectionGenericPrepare,
            BinaryLifecycleMutation::OperatorDuplicatePrepare,
        ] {
            let root = unique_tmp(&format!("runtime-binary-{mutation:?}"));
            copy_workspace_sources(&root, &[SERVER_MAIN_PATH, RSS_MAIN_PATH])?;
            let (relative, before, after) = match mutation {
                BinaryLifecycleMutation::ServerDiscardedPrepare => (
                    SERVER_MAIN_PATH,
                    "let runtime_inputs = runtime::prepare_runtime()?;",
                ...
                BinaryLifecycleMutation::OperatorDuplicatePrepare => (
                    RSS_MAIN_PATH,
                    "let runtime_inputs = runtime::operator::prepare_runtime()?;",
                    "let _discarded = runtime::operator::prepare_runtime()?;\n    let runtime_inputs = runtime::operator::prepare_runtime()?;",
                ),
            };
            let path = root.join(relative);
            let canonical = fs::read_to_string(&path)?;
            let mutated = canonical.replacen(before, after, 1);
            anyhow::ensure!(mutated != canonical, "{mutation:?} mutation must be live");
            write(&path, &mutated)?;
            let findings = runtime_binary_config_findings(&root)?;
            assert!(
                findings.iter().any(|finding| {
                    finding.rule == Rule::ForbiddenWiring && finding.subject == relative
                }),
                "real binary checker must reject {mutation:?}: {findings:?}"
            );
        }
        Ok(())
    }
    ...
    #[derive(Debug, Clone, Copy)]
    enum SecretTransferMutation {
        ExtraMove,
        ProjectionWrongReceiver,
        ExtraRawExtractor,
    }

    const SECRET_TRANSFER_SOURCES: &[&str] = &[
        RUNTIME_LIB_PATH,
        RUNTIME_SECRET_CONFIG_PATH,
        RUNTIME_EVENT_PATH,
        RUNTIME_VAULT_PATH,
    ...
    #[test]
    fn runtime_secret_transfer_checker_accepts_workspace() -> Result<()> {
        assert_eq!(
            runtime_secret_transfer_live_findings(&workspace_root()?)?,
            Vec::<Finding<Rule>>::new()
        );
        Ok(())
    }
    ...
    #[test]
    fn runtime_secret_transfer_checker_rejects_closed_mutations() -> Result<()> {
        for mutation in [
            SecretTransferMutation::ExtraMove,
            SecretTransferMutation::ProjectionWrongReceiver,
            SecretTransferMutation::ExtraRawExtractor,
        ] {
            let root = unique_tmp(&format!("runtime-secret-{mutation:?}"));
            copy_workspace_sources(&root, SECRET_TRANSFER_SOURCES)?;
            let (relative, mutated) = match mutation {
                SecretTransferMutation::ExtraMove => {
                    let canonical = fs::read_to_string(root.join(RUNTIME_EVENT_PATH))?;
                    (
                        RUNTIME_EVENT_PATH,
                        format!(
                            "{canonical}\nfn forbidden_secret_move(secret: EnvSecret) {{ let _ = secret.transfer_secret_allocation(); }}\n"
                        ),
                    )
                }
                SecretTransferMutation::ProjectionWrongReceiver => {
                    ...
                    anyhow::ensure!(mutated != canonical, "projection mutation must be live");
                    (RUNTIME_EVENT_PATH, mutated)
                }
                SecretTransferMutation::ExtraRawExtractor => {
                    let canonical = fs::read_to_string(root.join(RUNTIME_SECRET_CONFIG_PATH))?;
                    let mutated = canonical.replacen(
                        "    pub(crate) fn transfer_secret_allocation(self) -> String {\n        self.0.into_string()\n    }\n}",
                        "    pub(crate) fn transfer_secret_allocation(self) -> String {\n        self.0.into_string()\n    }\n\n    pub(crate) fn leaked_copy(&self) -> String {\n        self.0.expose().to_owned()\n    }\n}",
                        1,
                    );
                    anyhow::ensure!(mutated != canonical, "extractor mutation must be live");
                    (RUNTIME_SECRET_CONFIG_PATH, mutated)
                }
            };
            write(&root.join(relative), &mutated)?;
            let findings = runtime_secret_transfer_live_findings(&root)?;
            let (expected_subject, expected_detail) = match mutation {
                SecretTransferMutation::ExtraMove => {
                    (RUNTIME_SRC_PATH, "missing/extra unregistered handoffs")
                }
                SecretTransferMutation::ProjectionWrongReceiver => {
                    (RUNTIME_EVENT_PATH, "event.projection.hot")
                }
                SecretTransferMutation::ExtraRawExtractor => {
                    (RUNTIME_SECRET_CONFIG_PATH, "missing or has extra")
                }
            };
            assert!(
                findings.iter().any(|finding| {
                ...
                "real secret checker must reject {mutation:?} precisely: {findings:?}"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn runtime_config_phase_helper_ambiguity_reaches_production_gate() -> Result<()> {
        let root = unique_tmp("runtime-config-phase-helper-ambiguity");
        copy_workspace_sources(
            &root,
            &[
                RUNTIME_LIB_PATH,
                RUNTIME_OPERATOR_PATH,
                ...
                RUNTIME_PHASE_INFRA_PATH,
                RUNTIME_PHASE_DOMAINS_PATH,
                RUNTIME_PHASE_DLX_PATH,
            ],
        )?;
        let infra_path = root.join(RUNTIME_PHASE_INFRA_PATH);
        let canonical = fs::read_to_string(&infra_path)?;
        let mutated =
            format!("{canonical}\nimpl ProvidersBuilt<'_> {{ fn duplicate_phase_owner() {{}} }}\n");
        write(&infra_path, &mutated)?;
        let findings = runtime_config_snapshot_live_findings(&root)?;
        assert!(
            findings.iter().any(|finding| {
                finding.rule == Rule::MissingStructuralEvidence
                    && finding.subject == RUNTIME_PHASE_INFRA_PATH
                    && finding.detail.contains("ambiguous inherent impl")
            }),
            "runtime config production gate must fail closed on helper ambiguity: {findings:?}"
        );
        Ok(())
    }

    #[test]
    fn runtime_config_phase_helper_contexts_fail_closed() -> Result<()> {
        for (context, call) in [
            ("cfg/test", "#[cfg(test)] Self::helper();"),
            ("conditional", "if enabled() { Self::helper(); }"),
            ("loop", "for _ in inputs { Self::helper(); }"),
            ("closure", "let _deferred = || Self::helper();"),
            ("async", "let _deferred = async { Self::helper(); };"),
            (
                "unawaited once funnel",
                "let _deferred = after_required_preflight(input, |_| Self::helper());",
            ),
            ("macro", "defer! { Self::helper() }"),
        ] {
            let source = format!(
                "impl ProvidersBuilt {{ fn build_infra() {{ {call} }} fn helper() {{ live_config_evidence(); }} }}"
            );
            let file = syn::parse_file(&source)?;
            assert!(
                matches!(
                    expand_inherent_phase_method(&source, &file, "ProvidersBuilt", "build_infra",),
                    Err(PhaseExpandError::NonDirectCall(_))
                ),
                "{context} helper evidence must fail closed"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn collect_report_rejects_wrong_profile_before_runtime_ast_scan() -> Result<()> {
        let repository = AssemblyFixtureBuilder::production_universe()?
            .profile("runtime", AssemblyProfile::Test)?
            .build()?;
        let error = match collect_report(repository.path()) {
            Ok(_) => anyhow::bail!("wrong profile unexpectedly reached runtime AST collection"),
            Err(error) => error,
        };
        let detail = format!("{error:#}");
        assert!(
            detail.contains("production assembly") || detail.contains("production identities"),
            "the real global ratchet must reject profile drift before AST reads: {detail}"
        );
        assert!(
            !detail.contains(RUNTIME_LIB_PATH),
            "AST source must not be touched before the profile ratchet: {detail}"
        );
        Ok(())
    }
}
