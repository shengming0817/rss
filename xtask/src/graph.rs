...
const PRESENTATION_HEADER_MERMAID: &str =
    "%% Generated on demand by `cargo xtask graph assembly`; presentation only.";
...
impl Options {
    ...
    fn format_opt(&self) -> Option<Format> {
        self.format.first().copied()
    }

    /// 拒绝重复 assembly / format；缺省输出 runtime 双格式 presentation。
    pub(crate) fn validate(&self) -> Result<()> {
        if self.assembly.len() > 1 {
            bail!("graph assembly 参数 --assembly 重复");
        }
        if self.format.len() > 1 {
            bail!("graph assembly 参数 --format 重复");
        }
        Ok(())
    }
}

fn parse_assembly_name(name: &str) -> std::result::Result<String, String> {
    validate_name(name).map_err(|err| err.to_string())?;
    Ok(name.to_owned())
}
...
fn validate_model(nodes: &[Node], edges: &[Edge]) -> Result<()> {
    if nodes.is_empty() || edges.is_empty() {
        bail!("graph model 必须包含 node 与 edge");
    }
...
fn render_mermaid(model: &GraphModel) -> Result<String> {
    let mut out = format!("{PRESENTATION_HEADER_MERMAID}\nflowchart LR\n");
...
fn generate_model_root(root: &Path, options: &Options, model: &GraphModel) -> Result<()> {
    ...
    let base = root
        .join("target/xtask")
        .join(format!("{assembly}-assembly"));
    let formats: &[Format] = match options.format_opt() {
...
mod tests {
    ...
    fn cli_is_closed_and_defaults_to_runtime_both_formats() -> anyhow::Result<()> {
        ...
        assert!(parse_options(&["--check"]).is_err());
        assert_eq!(
            parse_options(&["--format", "json"])?.format_opt(),
            Some(Format::Json)
        );
        assert!(parse_options(&["--format", "yaml"]).is_err());
        assert!(parse_options(&["--format"]).is_err());
        assert!(parse_options(&["--assembly"]).is_err());
        assert!(parse_options(&["--assembly", "../runtime"]).is_err());
        assert!(parse_options(&["--format", "json", "--format", "mermaid"]).is_err());
        assert!(parse_options(&["--assembly", "runtime", "--assembly", "runtime"]).is_err());
        Ok(())
    }

    #[test]
    fn on_demand_runtime_graph_writes_only_target_and_overwrites() -> anyhow::Result<()> {
        let root = fixture_root("assembly-graph-on-demand-output")?;
        write_fixture(&root)?;
        crate::assembly_codegen::generate_root(&root, false)?;
        let target = root.join("target/xtask");
        std::fs::create_dir_all(&target)?;
        let json = target.join("runtime-assembly.json");
        let mermaid = target.join("runtime-assembly.mmd");
        std::fs::write(&json, "not owned\n")?;
        std::fs::write(&mermaid, "not owned\n")?;

        generate_fixture_graph_without_cargo(&root, &Options::default())?;
        generate_fixture_graph_without_cargo(
            &root,
            &Options {
                assembly: vec!["settingsonly".into()],
                ..Options::default()
            },
        )?;

        assert!(std::fs::read_to_string(&json)?.contains("\"assembly\""));
        assert!(std::fs::read_to_string(&mermaid)?.contains("flowchart LR"));
        assert!(target.join("settingsonly-assembly.json").is_file());
        assert!(target.join("settingsonly-assembly.mmd").is_file());
        assert!(!root.join("docs/architecture/generated").exists());
    ...
    fn parse_options(args: &[&str]) -> Result<Options> {
        use clap::Parser;
        #[derive(Parser)]
        #[command(name = "graph-assembly")]
        struct Wrapper {
            #[command(flatten)]
            options: Options,
        }
        let wrapper =
            Wrapper::try_parse_from(std::iter::once("graph-assembly").chain(args.iter().copied()))?;
        wrapper.options.validate()?;
        Ok(wrapper.options)
    }
    ...
    #[test]
    fn model_validation_rejects_invalid_sources_and_relations() -> anyhow::Result<()> {
        let root = fixture_root("assembly-graph-invalid-source")?;
        write_fixture(&root)?;
        crate::assembly_codegen::generate_root(&root, false)?;
        let manifest = root.join("assemblies/runtime/assembly.toml");
        let changed = std::fs::read_to_string(&manifest)?
            .replace("purpose = \"test\"", "purpose = \"changed\"");
        std::fs::write(&manifest, changed)?;
        let error = build_fixture_model_without_cargo(&root, "runtime")
            .expect_err("modules carrier drift must fail graph construction");
        assert!(format!("{error:#}").contains("modules_gen carrier"));
        std::fs::remove_dir_all(root)?;
        ...
        let duplicate = validate_model(&[node.clone(), node.clone()], &[self_edge]);
        assert!(
            format!("{:#}", duplicate.expect_err("duplicate node accepted"))
                .contains("node id 重复")
        );
        let dangling = validate_model(
            &[node],
            &[Edge {
                from: "node".into(),
                to: "missing".into(),
        ...
        assert!(
            format!("{:#}", dangling.expect_err("dangling edge accepted")).contains("悬空 edge")
        );
        Ok(())
    }
    ...
    #[test]
    fn real_runtime_graph_is_non_empty() -> anyhow::Result<()> {
