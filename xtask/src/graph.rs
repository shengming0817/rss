//! INVARIANT: ASSEMBLY-GRAPH-CODEGEN-01 { level = "Hard", exec = "verify", source = "codegen", golden = "docs/architecture/generated/runtime-assembly.json", synthetic_red = "graph::tests::check_rejects_every_source_mutation_without_writing", anti_vacuity = "graph::tests::committed_runtime_graph_is_non_empty_and_check_clean" } —— JSON/Mermaid 共用闭合 typed model，committed runtime 双产物由字节级 check 守漂移。
//! INVARIANT: ASSEMBLY-GRAPH-SOURCE-CLOSURE-01 { level = "Medium", exec = "verify", source = "code", synthetic_red = "graph::tests::check_rejects_every_source_mutation_without_writing", anti_vacuity = "graph::tests::model_and_renderers_are_deterministic_and_non_vacuous" } —— manifest、modules carrier 与 active event subscriptions 必须闭合，门具备非空真实证据与 synthetic red。
