...
impl Options {
    ...
    /// `--check` 仅 runtime；禁与 `--format` 同用；拒绝重复 flag。
    pub(crate) fn validate(&self) -> Result<()> {
        ...
        if self.check > 1 {
            bail!("graph assembly 参数 --check 重复");
        }
        if self.is_check() && self.assembly_name() != "runtime" {
            bail!("--check 仅适用于 committed runtime assembly graph");
        }
        if self.is_check() && self.format_opt().is_some() {
...
mod tests {
    ...
    fn cli_is_closed_and_defaults_to_runtime_both_formats() -> anyhow::Result<()> {
        ...
        assert!(parse_options(&["--check"])?.is_check());
        assert_eq!(
            parse_options(&["--format", "json"])?.format_opt(),
            Some(Format::Json)
        );
        assert!(parse_options(&["--assembly", "settingsonly", "--check"]).is_err());
        assert!(parse_options(&["--check", "--format", "json"]).is_err());
        assert!(parse_options(&["--format", "mermaid", "--check"]).is_err());
        ...
        assert!(parse_options(&["--check", "--check"]).is_err());
