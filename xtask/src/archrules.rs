...
pub(crate) fn matrix(facts: &WorkspaceFacts) -> Result<()> {
    let root = workspace_root()?;
    let index = build_index(&root, Some(facts))?;
    let mut findings = index.findings;
    findings.extend(validate_matrix(
    ...
    let path = write_matrix_report(&root, &rendered)?;
    eprintln!("archrules matrix: 已写入 {}", path.display());
    Ok(())
}

fn write_matrix_report(root: &Path, rendered: &str) -> Result<PathBuf> {
    let path = root.join(MATRIX_REPORT);
    let parent = path.parent().context("matrix report 缺父目录")?;
    fs::create_dir_all(parent)
        .with_context(|| format!("创建 matrix report 目录 `{}`", parent.display()))?;
    fs::write(&path, rendered)
        .with_context(|| format!("写入 matrix report `{}`", path.display()))?;
    Ok(path)
}
...
fn render_matrix(records: &[RuleRecord]) -> Result<String> {
    let mut out = String::from(
        "<!-- Generated on demand by `cargo xtask archrules matrix`; presentation only. -->\n\
# Persistence Funnel AI-Robust Matrix\n\n\
本报告只引用真实 `INVARIANT` carrier；level、carrier、gate、source 与 evidence 均由 ArchRules 索引派生，不参与 identity、equality 或 verify/ci verdict。\n\n\
...
|---|---|---|---|---|\n",
    ...
    out.push_str(&format!(
        "\n## Verification\n\n\
`cargo xtask archrules verify` 在内存中校验 source issue stable-ID {} 精确覆盖、key 唯一、边界非空、无 Soft、Hard carrier 证明与 Medium synthetic-red/anti-vacuity；该语义检查随 `archrules` 进入 `verify`/`ci`。\n",
        expected_issue_partition("–")
    ));
    Ok(out)
}
...
"#,
    ...
    #[test]
    fn real_workspace_archrules_semantics_and_on_demand_report_pass() -> Result<()> {
        let root = crate::workspace_root()?;
    ...
    #[test]
    fn on_demand_matrix_report_writes_only_target_and_overwrites() -> Result<()> {
        let root = unique_tmp("archrules-on-demand-report");
        let expected = root.join("target/xtask/persistence-funnel-ai-robust-matrix.md");

        let path = write_matrix_report(&root, "first\n")?;
        assert_eq!(path, expected);
        assert_eq!(fs::read_to_string(&path)?, "first\n");

        let path = write_matrix_report(&root, "second\n")?;
        assert_eq!(path, expected);
