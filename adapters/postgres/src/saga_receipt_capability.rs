...
// Generated from `SAGA_RECEIPT_CATALOG_SQL` against the real catalog produced by migration 0083.
const EXPECTED_SAGA_RECEIPT_CATALOG_FINGERPRINT: &str =
    "sha256:78b79dbc01ae1fa01866c71f55446a5ee594022d1feaec153502cc9e0835bfa3";
...
WITH receipt_table AS (
    SELECT relation.oid, relation.relowner, relation.relrowsecurity, relation.relforcerowsecurity
    FROM pg_catalog.pg_class AS relation
    JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = relation.relnamespace
    WHERE namespace.nspname = 'public'
      AND relation.relname = 'saga_step_receipts'
    ...
    JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = relation.relnamespace
    WHERE namespace.nspname = 'public'
      AND relation.relname IN ('saga_instances', 'saga_journal', 'saga_step_receipts')
    ...
    SELECT 'table:saga_step_receipts:rls=' || table_.relrowsecurity::text ||
           ':force=' || table_.relforcerowsecurity::text AS fact
    FROM receipt_table AS table_
    ...
    WHERE namespace.nspname = 'public'
      AND (
          relation.relname = 'saga_step_receipts'
          OR constraint_.conname IN (
              'saga_instances_terminal_time_consistent',
              'saga_instances_exact_identity_unique'
          )
      )
    ...
    WHERE namespace.nspname = 'public'
      AND index_.relname = 'saga_instances_terminal_retention_idx'
    ...
    SELECT 'function:' || function_.proname || '(' || function_.identity_arguments || ')' ||
           ':owner=' || CASE WHEN function_.proname = 'rss_sweep_terminal_sagas'
                             THEN pg_catalog.pg_get_userbyid(function_.proowner)
                             ELSE '<migration-owner>' END ||
...
impl VerifiedPgWriteStore {
    /// Mint the receipt only after the complete reviewed catalog surface matches migration 0083.
