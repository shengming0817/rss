...
//! - [`RedactingSpanExporter`]：导出最外边界经 `secure::redact_field` funnel 清洗 span 属性（防凭据/PII 裸传）。
...
/// 导出边界脱敏 exporter wrapper：span/event/link 属性进入下游 OTLP exporter 前，按
/// [`secure::redact_field`](secure::redact_field) 敏感 key 单源 funnel 清洗 string 值
/// （`authorization`/`access_token`/`session`/`cookie`/`jwt`… → `<redacted>`）——杜绝业务 span 字段把
/// 凭据 / PII 经 `tracing`→OTLP 裸传到 collector（`observability.md` §redaction；脱敏落在导出最外边界，
/// 覆盖所有属性来源，fail-closed）。
#[derive(Debug)]
...
/// 就地按敏感 key funnel 清洗一组属性的 string 值（非 string 值无自由文本 PII，跳过——
/// 避免把计数 / 布尔等 typed 值误改成字符串）。
fn redact_attributes(attributes: &mut [KeyValue]) {
    for kv in attributes {
        ...
        let replacement = {
            ...
            let redacted = redact_field(kv.key.as_str(), current.as_ref());
            (redacted.as_str() != current.as_ref()).then(|| redacted.as_str().to_owned())
        };
