//! redis — RSS workspace crate (signature-frozen; RW-G0.2 PR-5). See docs/rules/architecture.md.
//!
...
/// Redis adapter（sealed-marker）。raw client（连接池 / 句柄）字段待 W 阶段接后端时填入，保持 `pub(crate)`。
...
mod smoke {
    ...
    //! INVARIANT: ADAPTER-PORT-FREEZE-09 —— sealed-marker impl 冻结的 diport DI port trait；去掉任一 impl 即编译失败（anti-vacuity）。
