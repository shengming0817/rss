//! softca — RSS workspace crate (signature-frozen; RW-G0.2 PR-5). See docs/rules/architecture.md.
//!
...
/// 软件 CA 证书签发 adapter（sealed-marker）。raw client（key 材料 / 句柄）字段待 W 阶段填入，保持 `pub(crate)`。
...
mod smoke {
    ...
    //! INVARIANT: ADAPTER-PORT-FREEZE-11 —— sealed-marker impl 冻结的 diport DI port trait；去掉任一 impl 即编译失败（anti-vacuity）。
