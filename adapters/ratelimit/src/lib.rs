//! ratelimit — RSS workspace crate (signature-frozen; RW-G0.2 PR-5). See docs/rules/architecture.md.
//!
...
/// 限流 adapter（sealed-marker）。raw client（限流器状态 / 后端句柄）字段待 W 阶段填入，保持 `pub(crate)`。
pub struct RateLimiter;

impl ManagedResource for RateLimiter {
    fn name(&self) -> &str {
        todo!()
    }
...
mod smoke {
    ...
    //! INVARIANT: ADAPTER-PORT-FREEZE-08 —— sealed-marker impl 冻结的 diport DI port trait；去掉任一 impl 即编译失败（anti-vacuity）。
    ...
    fn impls_frozen_ports() {
        assert_managed_resource(PhantomData::<super::RateLimiter>);
    }
}
