//! prometheus — RSS workspace crate (signature-frozen; RW-G0.2 PR-5). See docs/rules/architecture.md.
//!
...
/// Prometheus metrics 导出 adapter（sealed-marker）。raw client（registry / exporter 句柄）字段待 W 阶段填入，保持 `pub(crate)`。
...
mod smoke {
    ...
    //! INVARIANT: ADAPTER-PORT-FREEZE-07 —— sealed-marker impl 冻结的 diport DI port trait；去掉任一 impl 即编译失败（anti-vacuity）。
