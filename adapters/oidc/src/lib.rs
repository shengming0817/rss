...
/// INVARIANT: ADAPTER-PORT-FREEZE-04 { level = "Hard", exec = "native-compile", source = "code", native = "sealed ManagedResource implementation on the production provider" }.
...
mod smoke {
    ...
    //! 不构造、不执行 body）。
    //! ADAPTER-PORT-FREEZE-04 support：sealed-marker impl 冻结的 diport DI port trait（`ManagedResource`
