...
| `[endpoints.http.projection]` | HTTP field projection 声明；`fields = [{ field = "auditActor", permission = "...", obligationKey = "...", responsePath = "data[].actor" }]`。`field` 是闭值集（当前 audit read + identity profile projection fields），`permission` / `obligationKey` / `responsePath` 必须非空且不重复。active GET response 中的 `x-pii` 字段与 `tenantId` 字段必须由 `responsePath` 精确覆盖；codegen 派生 typed `HttpProjectionFieldSpec`，handler/authorizer 只消费 `vocab::ProjectionField` | 按 endpoint（R23：protected read response field 必须 enrollment） |
