...
ci-full:
	...
	@set -eu; base="$$(/usr/bin/git rev-parse --verify "$(CI_BASE)^{commit}")"; head="$$(/usr/bin/git rev-parse --verify "$(CI_HEAD)^{commit}")"; if [ "$$base" = "$$head" ]; then base="$$(/usr/bin/git rev-parse --verify "$$head^")"; fi; baseline_packages="$$(/usr/bin/git show "$$base:Cargo.toml" | python3 -c 'import sys,tomllib; entries=tomllib.loads(sys.stdin.read())["workspace"]["metadata"]["release-surface"]["packages"]; print("\n".join(sorted(entry["package"] for entry in entries)))')"; current_metadata="$$(cargo metadata --locked --no-deps --format-version 1)"; library_packages="$$(printf '%s\n' "$$current_metadata" | python3 -c 'import json,sys; print("\n".join(sorted(package["name"] for package in json.load(sys.stdin)["packages"] if any("lib" in target["kind"] for target in package["targets"]))))')"; printf '%s\n' "$$current_metadata" | python3 -c 'import json,sys; entries=json.load(sys.stdin)["metadata"]["release-surface"]["packages"]; print("\n".join(sorted(entry["package"] for entry in entries)))' | while read -r package; do if ! printf '%s\n' "$$library_packages" | grep -Fqx "$$package"; then echo "semver-checks: skipping non-library target for $$package"; elif printf '%s\n' "$$baseline_packages" | grep -Fqx "$$package"; then cargo semver-checks check-release --package "$$package" --baseline-rev "$$base"; cargo semver-checks check-release --package "$$package" --all-features --baseline-rev "$$base"; else echo "semver-checks: skipping first Release Surface version for $$package"; fi; done
